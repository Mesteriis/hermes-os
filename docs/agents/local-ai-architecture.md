...
| Retrieval planner | selects graph, text and vector queries |
...
Models must not directly mutate durable state. Mutations pass through commands that validate provenance, confidence and permissions.
...
Private user data must not be used for fine-tuning. Durable memory belongs in events, graph, indexes and structured projections so the user can replace any model without losing history.
