...
| Polygraph engine is partially implemented. | Structured direct contradictions can be stored as reviewable observations, and guarded backend routes can list/review them without overwriting Memory. Communications/Documents are not yet wired into claim extraction and there is no review UI. | Connect extraction to `backend/src/engines/consistency.rs`, then add desktop review UI. |
| Decisions and Obligations are partial top-level domains. | Both have source-backed persistence, and Obligation has a first candidate detector. Message task candidate refresh uses that detector for explicit commitments/requests, and accepted Obligations/Decisions have guarded backend list/review routes. Candidate-to-Obligation/Decision review, provider-wide ingestion, meeting outcomes, project review decisions and follow-ups can still blur together. | Wire candidate extraction and review workflows to accepted Decisions and Obligations, then add adapters from compatibility surfaces. |
