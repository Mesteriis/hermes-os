...
| Polygraph engine is partially implemented. | Structured direct contradictions can be stored as reviewable observations, deterministic claim lines can be extracted from Communication/Document evidence text, and guarded backend routes can list/review observations without overwriting Memory. Provider-wide ingestion, natural-language claim extraction and review UI are not yet complete. | Connect ingestion to `backend/src/engines/consistency.rs`, then add desktop review UI. |
