...
| Relationships are not a complete first-class model. | Roles, organization links, graph edges and relationship events are spread across domains. | Define Relationship domain spec, then plan durable Relationship records with provenance, confidence, trust and validity. |
| Polygraph engine is partially implemented. | Structured direct contradictions can be stored as reviewable observations, but Communications/Documents are not yet wired into claim extraction and there is no review UI. | Connect extraction to `backend/src/engines/consistency.rs`, then add review routes and desktop UI. |
