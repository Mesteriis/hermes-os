...
| Relationships are not a complete first-class model. | Roles, organization links, graph edges and relationship events are spread across domains. Durable Relationship records, graph projection for supported endpoints, guarded backend review routes and scoped Personas workspace review have a baseline. | Expand desktop review beyond selected Personas, add remaining graph node kinds/adapters and migrate compatibility surfaces behind Relationship records. |
