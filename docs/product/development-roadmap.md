...
| Persona target model is not implemented end-to-end. | Current `persons` compatibility still carries contact/person history. | Plan Persona-native naming, Owner Persona, PersonaType and schema/API compatibility strategy. |
| Relationships are not a complete first-class model. | Roles, organization links, graph edges and relationship events are spread across domains. Durable Relationship records and guarded backend review routes have a baseline. | Add desktop review UI, expand projection/adapters and migrate compatibility surfaces behind Relationship records. |
| Polygraph engine is partially implemented. | Structured direct contradictions can be stored as reviewable observations, deterministic claim lines can be extracted from Communication/Document/Event evidence text, projected Communication message refresh, imported Document refresh, meeting-note refresh and call-transcript refresh can compare active `person_facts` Memory claims against structured evidence by Persona email identity, event participant link or active Telegram identity, and guarded backend routes can list/review observations without overwriting Memory. Non-email message-channel wiring, natural-language claim extraction and review UI are not yet complete. | Expand ingestion refresh in `backend/src/engines/consistency.rs`, then add desktop review UI. |
| Decisions and Obligations are partial top-level domains. | Both have source-backed persistence, deterministic candidate detectors where explicit evidence exists, accepted graph projection, and guarded backend list/review routes. Message task candidate refresh uses Obligation detection for explicit commitments/requests, confirmed `obligation_task` candidates now materialize accepted Obligations linked to Tasks, and explicit message/imported-document Decision candidates now persist as source-backed `suggested` Decisions. Provider-wide ingestion, meeting outcomes, project review decisions and follow-ups can still blur together. | Wire remaining candidate extraction and review workflows to accepted Decisions and Obligations, then add adapters from compatibility surfaces. |
...
- migrate `person_roles`, `person_personas` and root trust/watchlist fields into
  Relationship or read-model concepts;
...
- map `person_promises`, meeting outcomes and task candidates into target
  concepts;
