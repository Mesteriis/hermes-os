...
|---|---|---|
| Persona-native model incomplete | `persons`, `person_id`, `person_roles`, `person_personas`, `person_promises` and `/api/v1/persons/*` still exist. Owner Persona, PersonaType, role-to-Relationship, interaction-context-to-Preference, enrichment trust-to-Relationship and notes-to-memory-card adapters have compatibility baselines. | Keep compatibility short-term. Plan Persona-native schema/API and UI naming under a dedicated migration plan. |
