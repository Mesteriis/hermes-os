...
- Opaque UUID `person_id` migration — текущий контракт зафиксирован в ADR-0056 как text ID `person:v1:email:{len}:{email}`; UUID migration требует отдельного ADR и миграционного плана.
