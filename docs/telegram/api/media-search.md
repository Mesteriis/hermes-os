...
|---|---|---|
| POST | `/api/v1/communications/provider-media/download` | Download TDLib file and persist completed local blob + Communication attachment row |
| POST | `/api/v1/communications/provider-media/upload` | Queue provider-side media send from a local `attachment_id` or `blob_id` through the durable Telegram command outbox |
| GET | `/api/v1/communications/provider-search/media?account_id=&provider_chat_id=&kind=&limit=` | Projected Telegram media gallery/filter with attachment download metadata for files-tab parity |
...
POST /api/v1/communications/provider-media/upload
  -> telegram_provider_write_commands(command_kind=send_media)
  -> TDLib sendMessage media builder
```

`/api/v1/communications/provider-media/upload` does not accept raw file bytes and does not call
...
|---|---|---|
| GET | `/api/v1/communications/provider-media` | Media gallery/search |
| GET | `/api/v1/communications/provider-media/{media_id}/preview` | Telegram workbench preview |
| GET | `/api/v1/communications/provider-media/search?q=&type=` | Provider/local media search |
| POST | `/api/v1/communications/provider-voice/send` | Voice message send |
| POST | `/api/v1/communications/provider-video-note/send` | Video note send |
...
- workspace dialog search UI backed by `GET /api/v1/communications/provider-conversations/search`;
- workspace message search UI backed by `GET /api/v1/communications/provider-search/messages`;
- explicit provider search command `POST /api/v1/communications/provider-search/provider`;
- workspace media search UI plus files tab gallery backed by `GET /api/v1/communications/provider-search/media`.
...
| Method | Path | Назначение |
|---|---|---|
| GET | `/api/v1/communications/provider-search/provider` | Не требуется: command endpoint реализован как POST для явного provider search. |

Current search contract notes:

- `GET /api/v1/communications/provider-conversations/search` searches projected Telegram chats by
  title and optional `account_id` scope.
- `GET /api/v1/communications/provider-search/messages` searches projected Telegram messages by
...
- `POST /api/v1/communications/provider-search/provider` now exposes provider search as explicit
...
- `GET /api/v1/communications/provider-search/media` is a projection-backed gallery/filter
