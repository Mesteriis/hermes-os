...
The first Version 2 slice is complete when Hermes Hub builds a deterministic, read-only Knowledge Graph projection from existing persons, communication messages and documents, exposes protected read APIs, and renders graph-backed desktop dashboard data.
...
- Idempotent projection from `persons`, `communication_messages` and `documents`.
