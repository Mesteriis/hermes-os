...
| Polygraph engine is partially implemented. | Migration `0062`, `backend/src/engines/consistency.rs` and `backend/src/engines/consistency_api.rs` provide structured direct-contradiction detection, deterministic structured claim extraction from Communication/Document evidence text, reviewable observations and guarded backend review routes. Desktop UI, provider-wide ingestion wiring and natural-language claim extraction are incomplete. | Cross-domain engine for contradiction observations and review items. | Connect ingestion to the extraction baseline, then add desktop review UI. |
