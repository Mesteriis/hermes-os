...
| Consistency / Contradiction Engine | `backend/src/engines/consistency.rs`, `backend/src/engines/consistency_api.rs`, migration `0062`, ADR-0085, ADR-0087 | Structured direct-contradiction detection, observation persistence and guarded backend review routes have a baseline, but desktop review UI and upstream Communication/Document claim extraction are incomplete. | Connect extraction to structured claims, then add desktop review UI without automatic overwrite. |
