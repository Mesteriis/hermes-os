...
| Relationships | `backend/src/domains/relationships/mod.rs`, `backend/src/domains/relationships/api.rs`, migrations `0060` and `0061`, graph core, person roles, organization contacts, task relations, project link reviews, Personas workspace scoped review panel | First-class Relationship persistence, graph projection for Persona, Communication, Document, Project, Decision and Obligation endpoints, guarded backend review routes and scoped selected-Persona desktop review have a baseline, but global review, remaining graph node kinds and compatibility adapters are incomplete. | Add graph node kinds for Organization, Task, Event and Knowledge before projecting those endpoints, then migrate roles/links/read models behind compatibility boundaries. |
...
| Obligation Engine | `backend/src/engines/obligation.rs`, `backend/src/domains/obligations/mod.rs`, `backend/src/domains/obligations/api.rs`, migrations `0063`, `0066` and `0067`, task candidates, task rules, communication extraction, Tasks workspace scoped review panel | Candidate detection, accepted Obligation persistence, accepted Obligation graph projection, guarded accepted-Obligation backend review routes, scoped desktop entity review and confirmed `obligation_task` candidate materialization have baselines. Provider-wide Communication ingestion, meeting/document adapters, global review inbox and compatibility adapters are incomplete. | Extend Communication/meeting/document ingestion to the engine and feed reviewed candidates to the Obligations domain without auto-creating Tasks outside explicit task-candidate review. |
...
| Decisions domain | `backend/src/domains/decisions/mod.rs`, `backend/src/domains/decisions/api.rs`, `backend/src/engines/decision.rs`, migrations `0064` and `0065`, meeting outcomes, project link review decisions, communication/document evidence, Tasks workspace scoped review panel | Accepted Decision persistence, deterministic explicit-Decision candidate extraction, explicit message/imported-document candidate persistence as `suggested` Decisions, accepted Decision graph projection, guarded accepted-Decision backend review routes and scoped desktop entity review have baselines. Provider-wide ingestion, meeting adapters, global review inbox and adapters from meeting outcomes/project reviews are incomplete. | Connect remaining meeting/communication/document candidates to the Decisions domain without auto-changing Projects, Tasks or Obligations. |
