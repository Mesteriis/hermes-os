...
`GET /readyz` must verify that the required backend-managed SQLx migrations are successfully applied, not only that PostgreSQL accepts `SELECT 1` or that expected tables exist.
...
- Readiness can catch schema drift or missing migrations before API handlers operate on missing tables or columns.
