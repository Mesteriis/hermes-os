...
The current desktop client and planned Android client are presentations of the
same local-first memory system. Mobile lifecycle, transport and UI differences
must not create separate durable truth or provider/domain contracts.
...
Personal Memory System
  with full operational communication experiences
  and operating-system-like surfaces
  for shared context, knowledge and action.
```
