...
pub struct PersonaExpertise {
    ...
    #[serde(alias = "endorsed_by_person_id")]
    pub endorsed_by_persona_id: Option<String>,
...
impl PersonaExpertiseStore {
    ...
    pub async fn list(
        ...
        let rows = sqlx::query(
            "SELECT id::text, persona_id, skill, domain, evidence, source, confidence::float8 AS confidence,
             last_verified_at, endorsed_by_persona_id, created_at, updated_at
             FROM persona_expertise WHERE persona_id = $1 ORDER BY confidence DESC",
        )
        .bind(persona_id)
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn upsert(
        ...
        let row = sqlx::query(
            "INSERT INTO persona_expertise (persona_id, skill, domain, source, confidence)
             VALUES ($1, $2, $3, $4, $5)
             ON CONFLICT DO NOTHING
             RETURNING id::text, persona_id, skill, domain, evidence, source, confidence::float8 AS confidence,
                       last_verified_at, endorsed_by_persona_id, created_at, updated_at",
        )
        .bind(persona_id)
        .bind(skill)
...
fn row_to_expertise(row: PgRow) -> Result<PersonaExpertise, PersonaExpertiseError> {
    Ok(PersonaExpertise {
        ...
        endorsed_by_persona_id: row.try_get("endorsed_by_persona_id")?,
