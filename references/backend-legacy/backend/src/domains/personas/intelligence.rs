...
#[derive(Clone)]
pub struct PersonaIntelligenceService {
    hub: Option<SharedAiHub>,
}

impl PersonaIntelligenceService {
    pub fn new(hub: Option<SharedAiHub>) -> Self {
        Self { hub }
    }

    pub fn heuristic_fingerprint(messages: &[PersonaMessage]) -> CommunicationFingerprint {
    ...
    ) -> Result<Option<CommunicationFingerprint>, PersonaIntelligenceError> {
    ...
    pub fn suggested_actions(fingerprint: &CommunicationFingerprint) -> Vec<String> {
        ...
        if let Some(ref tone) = fingerprint.typical_tone {
            actions.push(format!(
                "Persona tends to be {tone} — match tone in replies"
            ));
        }
        ...
        {
            actions.push(format!(
                "Persona writes in {lang} — consider translating replies"
            ));
        }
        if let Some(ref style) = fingerprint.writing_style {
            actions.push(format!("Persona style: {style}"));
        }
...
#[derive(Debug, Error)]
pub enum PersonaIntelligenceError {
    #[error(transparent)]
    Hub(#[from] AiHubError),
    #[error(transparent)]
...
mod tests {
    use super::*;
    fn sample_messages() -> Vec<PersonaMessage> {
        vec![
            PersonaMessage {
    ...
    fn fingerprint_detects_topics() {
        let fp = PersonaIntelligenceService::heuristic_fingerprint(&sample_messages());
    ...
    fn empty_messages_returns_none() {
        let fp = PersonaIntelligenceService::heuristic_fingerprint(&[]);
