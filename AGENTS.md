...
- clean-room Cargo workspace в `backend/` с six-package Kernel foundation,
  private module control plane и five-package `vault_foundation_v1`; полный
  `vault_v1`, Clock и data-plane gates ещё закрыты;
...
- текущий production inventory закрыт exact allowlist `vault_foundation_v1`:
  six-package Kernel foundation плюс пять Vault foundation packages; Kernel не
  достигает `ready`, а `vault_v1`, Clock и data-plane gates остаются закрыты;
