...
- clean-room virtual Cargo workspace в `backend/` (пока без production
  package или runtime);
...
`docs/` оставлены только clean-room ADR и три architecture summaries. Всё в
...
| Architecture boundaries | `docs/adr/ADR-0200-clean-room-module-model-and-runtime-isolation.md`, `docs/adr/ADR-0201-core-module-communication-and-nats.md`, `docs/adr/ADR-0204-bundled-integration-plugins-and-provider-neutral-context-boundary.md`, `docs/adr/ADR-0205-core-gateway-and-client-transport.md`, `docs/adr/ADR-0206-kernel-constitution-boot-and-recovery-state-machine.md`, `docs/adr/ADR-0211-backend-workspace-and-source-layout.md`, `backend/architecture/policy.json`, `docs/architecture/component-communication.md` |
| Storage, PostgreSQL или pooling | `docs/adr/ADR-0202-postgresql-ownership-pgbouncer-and-extensions.md`, `docs/adr/ADR-0203-managed-infrastructure-supervision-and-recovery.md` |
| Runtime lifecycle, Kernel или recovery | `docs/adr/ADR-0200-clean-room-module-model-and-runtime-isolation.md`, `docs/adr/ADR-0203-managed-infrastructure-supervision-and-recovery.md`, `docs/adr/ADR-0206-kernel-constitution-boot-and-recovery-state-machine.md` |
| Event spine / envelope | `docs/adr/ADR-0201-core-module-communication-and-nats.md`; точный envelope contract ещё не создан |
...
| Secrets / vault | `docs/adr/ADR-0203-managed-infrastructure-supervision-and-recovery.md`, `docs/adr/ADR-0205-core-gateway-and-client-transport.md`, `docs/adr/ADR-0206-kernel-constitution-boot-and-recovery-state-machine.md`; trust model ещё не создан |
| AI / embeddings / agents | active clean-room specification отсутствует; требуется отдельный ADR |
...
policy. Первый production crate по-прежнему запрещён до согласования capability
и domain ownership inventory.

## 8. Architecture communication contract

Active policy находится в ADR-0200…ADR-0211 и executable companion
...
- durable commands, events, observations и results идут через PostgreSQL
  outbox/inbox и NATS JetStream;
- module runtime использует собственную PostgreSQL role через PgBouncer;
- ядро маршрутизирует envelopes и управляет lifecycle, но не интерпретирует
  business payload и не становится generic SQL proxy;
- provider experiences и runtimes поставляются только как bundled allowlisted
  plugins; store, runtime download и remote frontend code запрещены;
...
- client query/request/command используют owner-specific ConnectRPC/Protobuf
  contracts, realtime — один replayable SSE stream;
...
Детальный canonical summary находится в
