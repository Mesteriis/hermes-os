//! Telegram-owned state boundary. It does not expose tables or storage to other owners.
...
#[derive(Default)]
pub struct TelegramPersistence {
    ...
    evidence_outbox: HashMap<String, CommunicationObservationDraft>,
...
impl TelegramPersistence {
    ...
    pub fn message_is_locally_visible(&self, message_id: &str) -> bool {
    ...
    pub fn enqueue_observation(&mut self, draft: CommunicationObservationDraft) -> bool {
        let operation_id = draft.operation_id.clone();
        if self.evidence_outbox.contains_key(&operation_id) {
            return false;
        }
        self.evidence_outbox.insert(operation_id, draft);
        true
    }

    pub fn pending_observations(&self) -> Vec<CommunicationObservationDraft> {
        self.evidence_outbox.values().cloned().collect()
    }

    pub fn acknowledge_observation(&mut self, operation_id: &str) -> bool {
        self.evidence_outbox.remove(operation_id).is_some()
    }
    ...
    pub fn operations_ready_for_retry(&self, account_id: &str) -> Vec<TelegramOperation> {
        self.operations
            .values()
            .filter(|operation| {
    ...
    pub fn claim_due_operations(
        ...
        limit: usize,
        worker_id: &str,
    ) -> Vec<(TelegramOperation, TelegramProviderCommand)> {
        let mut candidates = self
            ...
            .filter(|operation| {
                operation.account_id == account_id
                    && matches!(
                        operation.state,
                        TelegramOperationState::Accepted | TelegramOperationState::RetryScheduled
                    )
                    && operation.retry_count < operation.max_retries
                    && operation
                        .next_attempt_at_unix_seconds
                        .is_none_or(|next| next <= now_unix_seconds)
            })
            .map(|operation| operation.operation_id.clone())
            .collect::<Vec<_>>();
        candidates.sort();
        ...
        for operation_id in candidates {
            ...
            let Some(command) = self.commands.get(&operation_id).cloned() else {
                continue;
            };
            operation.state = TelegramOperationState::Running;
            operation.retry_count = operation.retry_count.saturating_add(1);
            operation.locked_at_unix_seconds = Some(now_unix_seconds);
            operation.locked_by = Some(worker_id.to_owned());
            operation.next_attempt_at_unix_seconds = None;
            operation.last_error = None;
            claimed.push((operation.clone(), command));
        }
        claimed
    }
    ...
    pub fn recover_stale_operations(
        &mut self,
        now_unix_seconds: u64,
            ...
            .filter(|operation| {
                operation.state == TelegramOperationState::Running
                    && operation
                        ...
                        .is_some_and(|locked| locked <= stale_before_unix_seconds)
            })
            .map(|operation| operation.operation_id.clone())
            .collect::<Vec<_>>();
        ids.into_iter()
            .filter_map(|operation_id| {
                ...
                operation.last_error = Some(
                    "Telegram provider command execution was interrupted before reconciliation"
                        .to_owned(),
                );
                if operation.retry_count >= operation.max_retries {
                    operation.state = TelegramOperationState::DeadLetter;
                    ...
                    operation.state = TelegramOperationState::RetryScheduled;
                    operation.next_attempt_at_unix_seconds = Some(now_unix_seconds);
                }
    ...
    pub fn remove_qr_session(&mut self, setup_id: &str) -> Option<TelegramQrLoginSession> {
        self.qr_sessions.remove(setup_id)
    }
    ...
    pub fn participants(
        ...
        account_id: &str,
        provider_chat_id: &str,
        filter: hermes_telegram_api::TelegramParticipantFilter,
    ) -> Option<&[TelegramParticipant]> {
        self.participants
            .get(&format!("{account_id}:{provider_chat_id}:{filter:?}"))
            .map(Vec::as_slice)
    }
...
mod tests {
    ...
    #[test]
    fn evidence_outbox_deduplicates_and_acknowledges_by_operation() {
        let mut persistence = TelegramPersistence::new();
        let draft = CommunicationObservationDraft {
            operation_id: "telegram:account:chat:message".to_owned(),
            source_id: "telegram:account:chat:message".to_owned(),
            source_kind: "telegram".to_owned(),
            text_preview: Some("hello".to_owned()),
            has_body: true,
            is_final_window: true,
        };
        assert!(persistence.enqueue_observation(draft.clone()));
        assert!(!persistence.enqueue_observation(draft));
        assert_eq!(persistence.pending_observations().len(), 1);
        assert!(persistence.acknowledge_observation("telegram:account:chat:message"));
        assert!(persistence.pending_observations().is_empty());
    }
