CREATE TABLE hermes_data.communications_retained_evidence_replay_command_inbox (
...
CREATE TABLE hermes_data.communications_retained_evidence_replay_result_outbox (
    message_id BYTEA PRIMARY KEY CHECK (octet_length(message_id) = 16),
    envelope_sha256 BYTEA NOT NULL CHECK (octet_length(envelope_sha256) = 32),
    ...
    FOREIGN KEY (command_message_id, operation_id) REFERENCES
        hermes_data.communications_retained_evidence_replay_command_inbox (
            message_id,
            operation_id
        )
);
...
ON hermes_data.communications_retained_evidence_replay_result_outbox (
