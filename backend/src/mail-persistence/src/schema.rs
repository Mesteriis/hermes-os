...
use crate::{
    ...
    MAIL_SCHEMA_V7, MAIL_SCHEMA_V8, MAIL_SCHEMA_V9,
};
...
mod tests {
    ...
    fn bundle_is_valid_and_owned_only_by_mail() {
        ...
        assert_eq!(bundle.revision, MAIL_STORAGE_BUNDLE_REVISION_V9);
        assert_eq!(validate_storage_bundle(&bundle), Ok(()));
        assert_eq!(bundle.steps.len(), 9);
        let sql = bundle
            .steps
            .iter()
            .map(|step| {
            ...
            .collect::<Vec<_>>()
            .join("\n");
        assert_eq!(sql.matches("CREATE TABLE IF NOT EXISTS ").count(), 23);
        assert_eq!(
