...
use crate::{
    ...
    MAIL_SCHEMA_V17, MAIL_SCHEMA_V18,
};
...
mod tests {
    ...
    fn bundle_is_valid_and_owned_only_by_mail() {
        ...
        assert_eq!(bundle.revision, MAIL_STORAGE_BUNDLE_REVISION_V18);
        assert_eq!(validate_storage_bundle(&bundle), Ok(()));
        assert_eq!(bundle.steps.len(), 18);
        let sql = bundle
            .steps
            .iter()
            .map(|step| {
            ...
            .collect::<Vec<_>>()
            .join("\n");
        assert_eq!(sql.matches("CREATE TABLE IF NOT EXISTS ").count(), 36);
        assert_eq!(
