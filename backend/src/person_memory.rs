...
impl PersonFactStore {
    ...
    pub async fn upsert(
        ...
        ).bind(person_id).bind(fact_type).bind(value).bind(source).bind(confidence)
         .fetch_one(&self.pool).await?;
        row_to_fact(row)
    }

    pub async fn update_confidence(&self, id: &str, confidence: f64) -> Result<(), PersonMemoryError> {
...
impl PersonMemoryCardStore {
    ...
    pub async fn upsert(
        &self, person_id: &str, title: &str, description: &str, source: &str, importance: i16,
        ...
        ).bind(person_id).bind(title).bind(description).bind(source).bind(importance)
         .fetch_one(&self.pool).await?;
...
fn row_to_memory_card(row: PgRow) -> Result<PersonMemoryCard, PersonMemoryError> {
    Ok(PersonMemoryCard {
        ...
        source: row.try_get("source")?, confidence: row.try_get("confidence")?,
        importance: row.try_get("importance")?, created_at: row.try_get("created_at")?,
        last_verified_at: row.try_get("last_verified_at")?,
    })
}
...
impl PersonPreferenceStore {
    ...
    pub async fn upsert(
        &self, person_id: &str, preference_type: &str, value: &str, source: &str,
...
fn row_to_preference(row: PgRow) -> Result<PersonPreference, PersonMemoryError> {
    Ok(PersonPreference {
        id: row.try_get("id")?, person_id: row.try_get("person_id")?,
        preference_type: row.try_get("preference_type")?, value: row.try_get("value")?,
        source: row.try_get("source")?, confidence: row.try_get("confidence")?,
        last_verified_at: row.try_get("last_verified_at")?, created_at: row.try_get("created_at")?,
        updated_at: row.try_get("updated_at")?,
    })
}
...
impl RelationshipEventStore {
    ...
    pub async fn timeline(
        ...
        ).bind(person_id).bind(limit.clamp(1, 100)).fetch_all(&self.pool).await?;
        rows.into_iter().map(row_to_event).collect()
    }

    pub async fn add(
        &self, event: &NewRelationshipEvent,
        ...
        let row = sqlx::query(
            "INSERT INTO relationship_events (person_id, event_type, title, description,
             ...
             RETURNING id::text, person_id, event_type, title, description, occurred_at, source,
                       related_entity_id, related_entity_kind, confidence, metadata, created_at"
        ).bind(&event.person_id).bind(&event.event_type).bind(&event.title)
         .bind(&event.description).bind(event.occurred_at).bind(&event.source)
         .bind(&event.related_entity_id).bind(&event.related_entity_kind)
         .fetch_one(&self.pool).await?;
        row_to_event(row)
    }
}
...
fn row_to_event(row: PgRow) -> Result<RelationshipEvent, PersonMemoryError> {
    Ok(RelationshipEvent {
        id: row.try_get("id")?, person_id: row.try_get("person_id")?,
        event_type: row.try_get("event_type")?, title: row.try_get("title")?,
        description: row.try_get("description")?, occurred_at: row.try_get("occurred_at")?,
        source: row.try_get("source")?, related_entity_id: row.try_get("related_entity_id")?,
        related_entity_kind: row.try_get("related_entity_kind")?,
        confidence: row.try_get("confidence")?, metadata: row.try_get("metadata")?,
        created_at: row.try_get("created_at")?,
    })
}
...
impl PersonSnapshotStore {
    ...
    pub async fn create(
        &self, person_id: &str, data: Value, source: &str,
        ...
        ).bind(person_id).bind(&data).bind(source).fetch_one(&self.pool).await?;
        row_to_snapshot(row)
    }

    /// Compare two snapshots and return the differences.
    pub async fn history_diff(
        &self, person_id: &str, from_date: DateTime<Utc>, to_date: DateTime<Utc>,
    ) -> Result<HistoryDiff, PersonMemoryError> {
        let from = sqlx::query(
            "SELECT id::text, person_id, snapshot_date, data, source, created_at
             FROM person_snapshots WHERE person_id = $1 AND snapshot_date <= $2
             ORDER BY snapshot_date DESC LIMIT 1"
        ).bind(person_id).bind(from_date).fetch_optional(&self.pool).await?;

        let to = sqlx::query(
            "SELECT id::text, person_id, snapshot_date, data, source, created_at
             FROM person_snapshots WHERE person_id = $1 AND snapshot_date <= $2
             ORDER BY snapshot_date DESC LIMIT 1"
        ).bind(person_id).bind(to_date).fetch_optional(&self.pool).await?;
