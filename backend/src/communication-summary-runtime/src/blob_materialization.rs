...
use hermes_communication_summary_persistence::CommunicationSummaryBlobCleanupV1;
use hermes_communications_ai_source_api::decode_communication_summary_source_content_v1;
...
fn ai_reference_id(run_id: [u8; 16], local_reference: [u8; 16], sha256: [u8; 32]) -> [u8; 16] {
    ...
    digest.update(b"hermes.communication_summary.ai-source.v1\0");
...
fn release_operation_id(run_id: [u8; 16], label: &[u8]) -> [u8; 16] {
    let mut digest = Sha256::new();
    digest.update(b"hermes.communication_summary.blob-release.v1\0");
    digest.update(label);
    digest.update(run_id);
    digest.finalize()[..16].try_into().expect("digest prefix")
}
