...
use hermes_review_task_candidate_assembly::materialize_review_task_candidate_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: hermes-review-task-candidate-assembly --build-id <id> \
