...
#[derive(Clone, Debug, Eq, PartialEq, serde::Serialize)]
pub(crate) struct WhatsAppBusinessCloudWebhookIngestResponse {
    pub(crate) processed_messages: usize,
    pub(crate) processed_statuses: usize,
    pub(crate) unsupported_events: usize,
    pub(crate) raw_record_ids: Vec<String>,
}

#[derive(Clone, Debug, Deserialize)]
pub(crate) struct WhatsAppBusinessCloudWebhookVerifyQuery {
...
#[derive(Clone, Debug, Eq, PartialEq, serde::Serialize)]
pub(crate) struct WhatsAppBusinessCloudProxyManifestResponse {
    pub(crate) provider_kind: &'static str,
    pub(crate) provider_shape: &'static str,
    pub(crate) protected_runtime_bridge_path: &'static str,
    pub(crate) proxy_manifest_path: &'static str,
    pub(crate) hermes_direct_public_endpoint: bool,
    pub(crate) edge_proxy_required: bool,
    pub(crate) local_auth_header: &'static str,
    pub(crate) local_auth_policy: &'static str,
    pub(crate) request_body_policy: &'static str,
    pub(crate) secret_material_policy: &'static str,
    pub(crate) event_flow: &'static str,
    pub(crate) route_contracts: Vec<WhatsAppBusinessCloudProxyRouteContract>,
    pub(crate) secret_purposes: WhatsAppBusinessCloudProxySecretPurposes,
    pub(crate) accounts: Vec<WhatsAppBusinessCloudProxyAccountState>,
}

#[derive(Clone, Debug, Eq, PartialEq, serde::Serialize)]
pub(crate) struct WhatsAppBusinessCloudProxyRouteContract {
    pub(crate) method: &'static str,
    pub(crate) protected_path: &'static str,
    pub(crate) required_query_params: Vec<&'static str>,
    pub(crate) optional_query_params: Vec<&'static str>,
    pub(crate) required_headers: Vec<&'static str>,
    pub(crate) body_forwarding: &'static str,
    pub(crate) verification: &'static str,
    pub(crate) response_policy: &'static str,
}

#[derive(Clone, Debug, Eq, PartialEq, serde::Serialize)]
pub(crate) struct WhatsAppBusinessCloudProxySecretPurposes {
    pub(crate) access_token: &'static str,
    pub(crate) app_secret: &'static str,
    pub(crate) webhook_verify_token: &'static str,
}

#[derive(Clone, Debug, Eq, PartialEq, serde::Serialize)]
pub(crate) struct WhatsAppBusinessCloudProxyAccountState {
    pub(crate) account_id: String,
    pub(crate) lifecycle_state: Option<String>,
    pub(crate) runtime: Option<String>,
    pub(crate) business_cloud_live_smoke_enabled: bool,
    pub(crate) access_token_bound: bool,
    pub(crate) app_secret_bound: bool,
    pub(crate) webhook_verify_token_bound: bool,
    pub(crate) webhook_proxy_ready: bool,
    pub(crate) command_submission_ready: bool,
    pub(crate) remaining_blockers: Vec<&'static str>,
}

#[derive(Clone, Debug, Default, Eq, PartialEq)]
struct WhatsAppBusinessCloudWebhookSecretBindings {
    app_secret_bound: bool,
    webhook_verify_token_bound: bool,
}
...
pub(crate) async fn post_whatsapp_account(
    ...
    let response = whatsapp_provider_runtime_service(&state)?
        ...
        .await?;
    let mut account = provider_account_or_not_found(&state, &response.account_id).await?;
    let webhook_secret_bindings =
        store_whatsapp_business_cloud_webhook_secrets(&state, &request, &account).await?;
    if store_whatsapp_business_cloud_access_token(&state, &request, &account)
        .await?
        .is_some()
    {
        account = mark_whatsapp_business_cloud_credentials_linked(
            &state,
            &account,
...
async fn store_whatsapp_business_cloud_webhook_secrets(
    state: &AppState,
    request: &WhatsappLiveAccountSetupRequest,
    account: &ProviderAccount,
) -> Result<WhatsAppBusinessCloudWebhookSecretBindings, ApiError> {
    if request.provider_shape.trim() != "whatsapp_business_cloud" {
        return Ok(WhatsAppBusinessCloudWebhookSecretBindings::default());
    }

    let app_secret = store_whatsapp_business_cloud_optional_secret(
        state,
        account,
        request.app_secret.as_deref(),
        ProviderAccountSecretPurpose::WhatsappBusinessCloudAppSecret,
        whatsapp_business_cloud_app_secret_ref(&account.account_id),
        "business_cloud_app_secret",
        "provider_webhook_secret",
        "WhatsApp Business Cloud app secret",
    )
    .await?;
    let webhook_verify_token = store_whatsapp_business_cloud_optional_secret(
        state,
        account,
        request.webhook_verify_token.as_deref(),
        ProviderAccountSecretPurpose::WhatsappBusinessCloudWebhookVerifyToken,
        whatsapp_business_cloud_webhook_verify_token_ref(&account.account_id),
        "business_cloud_webhook_verify_token",
        "provider_webhook_secret",
        "WhatsApp Business Cloud webhook verify token",
    )
    .await?;

    Ok(WhatsAppBusinessCloudWebhookSecretBindings {
        app_secret_bound: app_secret.is_some(),
        webhook_verify_token_bound: webhook_verify_token.is_some(),
    })
}

async fn store_whatsapp_business_cloud_access_token(
    state: &AppState,
    request: &WhatsappLiveAccountSetupRequest,
    account: &ProviderAccount,
) -> Result<Option<WhatsAppCredentialBinding>, ApiError> {
    if request.provider_shape.trim() != "whatsapp_business_cloud" {
        return Ok(None);
    }
    ...
    let token = request
        ...
        .ok_or_else(|| {
            WhatsappWebError::InvalidRequest(
                "api_access_token is required for whatsapp_business_cloud".to_owned(),
            )
        })?;
    store_whatsapp_business_cloud_required_secret(
        state,
        account,
        "api_access_token",
        token,
        ProviderAccountSecretPurpose::WhatsappBusinessCloudAccessToken,
        whatsapp_business_cloud_access_token_secret_ref(&account.account_id),
        "business_cloud_access_token",
        "provider_api_token",
...
async fn store_whatsapp_business_cloud_required_secret(
    state: &AppState,
    account: &ProviderAccount,
    field_name: &str,
    value: &str,
    ...
    if secret.is_empty() {
        return Err(WhatsappWebError::InvalidRequest(format!(
            "{field_name} is required for whatsapp_business_cloud"
        ))
        .into());
    }
    store_whatsapp_business_cloud_secret(
        state,
        account,
        purpose,
        secret_ref,
...
#[allow(clippy::too_many_arguments)]
async fn store_whatsapp_business_cloud_optional_secret(
    state: &AppState,
    account: &ProviderAccount,
    value: Option<&str>,
    purpose: ProviderAccountSecretPurpose,
...
#[allow(clippy::too_many_arguments)]
async fn store_whatsapp_business_cloud_secret(
    state: &AppState,
    account: &ProviderAccount,
    purpose: ProviderAccountSecretPurpose,
    secret_ref: String,
    ...
    label: &str,
    secret: &str,
) -> Result<WhatsAppCredentialBinding, ApiError> {
    let metadata = json!({
        ...
        "app_secret": "excluded",
        "webhook_verify_token": "excluded",
    });
    let secret_store = whatsapp_secret_reference_store(state)?;
    secret_store
        ...
        .await
        .map_err(WhatsappWebError::SecretReference)?;
...
async fn mark_whatsapp_business_cloud_credentials_linked(
    state: &AppState,
    account: &ProviderAccount,
    webhook_secret_bindings: &WhatsAppBusinessCloudWebhookSecretBindings,
) -> Result<ProviderAccount, ApiError> {
    let mut config = account.config.clone();
    let config_object = config.as_object_mut().ok_or_else(|| {
        WhatsappWebError::InvalidRequest("WhatsApp account config must be a JSON object".to_owned())
    })?;
    config_object.insert("lifecycle_state".to_owned(), json!("linked"));
    config_object.insert(
        "business_cloud_credentials".to_owned(),
        json!({
            "access_token_bound": true,
            "secret_purpose": ProviderAccountSecretPurpose::WhatsappBusinessCloudAccessToken.as_str(),
            "store_kind": SecretStoreKind::HostVault.as_str(),
            "app_secret_bound": webhook_secret_bindings.app_secret_bound,
            "app_secret_purpose": ProviderAccountSecretPurpose::WhatsappBusinessCloudAppSecret.as_str(),
            "webhook_verify_token_bound": webhook_secret_bindings.webhook_verify_token_bound,
            "webhook_verify_token_purpose": ProviderAccountSecretPurpose::WhatsappBusinessCloudWebhookVerifyToken.as_str(),
            "secret_material": "excluded",
            "updated_at": Utc::now(),
        }),
    );
...
pub(crate) async fn get_whatsapp_runtime_bridge_business_cloud_proxy_manifest(
    State(state): State<AppState>,
) -> Result<Json<WhatsAppBusinessCloudProxyManifestResponse>, ApiError> {
    let protected_runtime_bridge_path =
        "/api/v1/integrations/whatsapp/runtime-bridge/business-cloud/webhooks";
    let proxy_manifest_path =
        "/api/v1/integrations/whatsapp/runtime-bridge/business-cloud/proxy-manifest";
    Ok(Json(WhatsAppBusinessCloudProxyManifestResponse {
        provider_kind: "whatsapp_business_cloud",
        provider_shape: "whatsapp_business_cloud",
        protected_runtime_bridge_path,
        proxy_manifest_path,
        hermes_direct_public_endpoint: false,
        edge_proxy_required: true,
        local_auth_header: "X-Hermes-Secret",
        local_auth_policy: "edge_proxy_injects_local_secret_only_on_127_0_0_1_request",
        request_body_policy: "post_raw_body_must_be_forwarded_byte_for_byte_before_json_parse",
        secret_material_policy: "secret_ref_bindings_only_secret_values_excluded",
        event_flow: "business_cloud_webhook -> verified runtime_bridge -> raw_evidence -> signal_hub_accepted -> projection_reconciliation",
        route_contracts: vec![
            WhatsAppBusinessCloudProxyRouteContract {
                method: "GET",
                protected_path: protected_runtime_bridge_path,
                required_query_params: vec!["hub.mode", "hub.verify_token", "hub.challenge"],
                optional_query_params: vec!["account_id"],
                required_headers: vec!["X-Hermes-Secret"],
                body_forwarding: "no_body",
                verification: "host_vault_whatsapp_business_cloud_webhook_verify_token",
                response_policy: "return_hub_challenge_only_after_verify_token_match",
            },
            WhatsAppBusinessCloudProxyRouteContract {
                method: "POST",
                protected_path: protected_runtime_bridge_path,
                required_query_params: Vec::new(),
                optional_query_params: Vec::new(),
                required_headers: vec!["X-Hermes-Secret", "X-Hub-Signature-256"],
                body_forwarding: "forward_exact_raw_body_do_not_inject_or_rewrite_payload",
                verification: "host_vault_whatsapp_business_cloud_app_secret_hmac_sha256",
                response_policy: "return_sanitized_ingest_counts_after_signal_hub_runtime_bridge_ingest",
            },
        ],
        secret_purposes: WhatsAppBusinessCloudProxySecretPurposes {
            access_token: ProviderAccountSecretPurpose::WhatsappBusinessCloudAccessToken.as_str(),
            app_secret: ProviderAccountSecretPurpose::WhatsappBusinessCloudAppSecret.as_str(),
            webhook_verify_token:
                ProviderAccountSecretPurpose::WhatsappBusinessCloudWebhookVerifyToken.as_str(),
        },
        accounts: business_cloud_proxy_manifest_accounts(&state).await?,
    }))
}

pub(crate) async fn get_whatsapp_runtime_bridge_business_cloud_webhook(
    State(state): State<AppState>,
    Query(query): Query<WhatsAppBusinessCloudWebhookVerifyQuery>,
    ...
    let verify_token = query
        ...
        .ok_or_else(|| {
            WhatsappWebError::InvalidRequest(
                "hub.verify_token is required for Business Cloud webhook verification".to_owned(),
            )
        })?;
    let challenge = query
        .challenge
        ...
        .ok_or_else(|| {
            WhatsappWebError::InvalidRequest(
                "hub.challenge is required for Business Cloud webhook verification".to_owned(),
            )
        })?;
    if mode != "subscribe" {
        return Err(WhatsappWebError::InvalidRequest(format!(
            "Unsupported Business Cloud webhook verification mode `{mode}`"
        ))
        .into());
    }
    verify_business_cloud_webhook_challenge_token(
        &state,
...
pub(crate) async fn post_whatsapp_runtime_bridge_business_cloud_webhook(
    State(state): State<AppState>,
    headers: HeaderMap,
    body: Bytes,
) -> Result<Json<WhatsAppBusinessCloudWebhookIngestResponse>, ApiError> {
    let payload: Value = serde_json::from_slice(&body).map_err(|error| {
        WhatsappWebError::InvalidRequest(format!(
            "Business Cloud webhook payload must be valid JSON: {error}"
        ))
    })?;
    let ingest_service = whatsapp_fixture_ingest_service(&state)?;
    let mut response = WhatsAppBusinessCloudWebhookIngestResponse {
        processed_messages: 0,
        processed_statuses: 0,
        unsupported_events: 0,
        raw_record_ids: Vec::new(),
    };
    let explicit_account_id =
        business_cloud_webhook_explicit_account_id(&payload).map(str::to_owned);
    let webhook_values = business_cloud_webhook_values(&payload);
    let verification_value = webhook_values.first().ok_or_else(|| {
        WhatsappWebError::InvalidRequest(
            "Business Cloud webhook payload must contain entry[].changes[].value messages/statuses"
                .to_owned(),
        )
    })?;
    let verification_account_id = business_cloud_webhook_account_id(
        &state,
        explicit_account_id.as_deref(),
        verification_value,
    )
    .await?;
    verify_business_cloud_webhook_signature(
        &state,
    ...
    for value in webhook_values {
        let account_id =
            business_cloud_webhook_account_id(&state, explicit_account_id.as_deref(), value)
                .await?;
        for message in value
            ...
            .flatten()
        {
            let Some(request) = business_cloud_webhook_message_request(&account_id, value, message)
            else {
                let raw_record_id = ingest_business_cloud_unsupported_webhook_event(
                    &ingest_service,
                    &account_id,
                    value,
                    message,
                    "unsupported_business_cloud_message",
                )
                .await?;
                response.unsupported_events += 1;
                response.raw_record_ids.push(raw_record_id);
                continue;
            };
            let result = ingest_service
                .ingest_runtime_bridge_message(&request)
                .await?;
            publish_whatsapp_projection_event(
                &state,
                ...
                json!({
                    ...
                    "sender_display_name": request.sender_display_name,
                    "occurred_at": request.occurred_at,
                    "source": "business_cloud_webhook_message_ingest",
                    "provider_shape": "whatsapp_business_cloud",
            ...
            response.raw_record_ids.push(result.raw_record_id);
        }
        ...
        for status in value
            ...
            .flatten()
        {
            let Some(request) = business_cloud_webhook_receipt_request(&account_id, status) else {
                let raw_record_id = ingest_business_cloud_unsupported_webhook_event(
                    &ingest_service,
                    &account_id,
                    value,
                    status,
                    "unsupported_business_cloud_status",
                )
                .await?;
                response.unsupported_events += 1;
                response.raw_record_ids.push(raw_record_id);
            ...
            publish_whatsapp_projection_event(
                ...
                json!({
                    ...
                    "delivery_state": result.delivery_state,
                    "observed_at": request.observed_at,
                    "source": "business_cloud_webhook_status_ingest",
                    "provider_shape": "whatsapp_business_cloud",
                    "payload_policy": "sanitized_metadata_only",
                }),
            )
            .await?;
            response.processed_statuses += 1;
            response.raw_record_ids.push(result.raw_record_id);
        }
    }
...
async fn business_cloud_proxy_manifest_accounts(
    state: &AppState,
) -> Result<Vec<WhatsAppBusinessCloudProxyAccountState>, ApiError> {
    let pool = state
        .database
        .pool()
        .ok_or(ApiError::DatabaseNotConfigured)?;
    let rows = sqlx::query(
        ...
        SELECT
            account.account_id,
            COALESCE(account.config->>'lifecycle_state', '') AS lifecycle_state,
            COALESCE(account.config->>'runtime', '') AS runtime,
            COALESCE(account.config->>'business_cloud_live_smoke_enabled', 'false')
                AS business_cloud_live_smoke_enabled,
            EXISTS (
                SELECT 1
                FROM communication_provider_account_secret_refs binding
                WHERE binding.account_id = account.account_id
                  AND binding.secret_purpose = $1
            ) AS access_token_bound,
            EXISTS (
                SELECT 1
                FROM communication_provider_account_secret_refs binding
                WHERE binding.account_id = account.account_id
                  AND binding.secret_purpose = $2
            ) AS app_secret_bound,
            EXISTS (
                SELECT 1
                FROM communication_provider_account_secret_refs binding
                WHERE binding.account_id = account.account_id
                  AND binding.secret_purpose = $3
            ) AS webhook_verify_token_bound
        FROM communication_provider_accounts account
        WHERE account.provider_kind = 'whatsapp_business_cloud'
          AND COALESCE(account.config->>'provider_shape', '') = 'whatsapp_business_cloud'
        ORDER BY account.updated_at DESC, account.account_id ASC
        "#,
    )
    .bind(ProviderAccountSecretPurpose::WhatsappBusinessCloudAccessToken.as_str())
    .bind(ProviderAccountSecretPurpose::WhatsappBusinessCloudAppSecret.as_str())
    .bind(ProviderAccountSecretPurpose::WhatsappBusinessCloudWebhookVerifyToken.as_str())
    .fetch_all(pool)
    .await
    .map_err(WhatsappWebError::Sqlx)?;

    rows.into_iter()
        .map(|row| {
            let account_id = row.try_get::<String, _>("account_id")?;
            let lifecycle_state = non_empty_string(row.try_get::<String, _>("lifecycle_state")?);
            let runtime = non_empty_string(row.try_get::<String, _>("runtime")?);
            let business_cloud_live_smoke_enabled = row
                .try_get::<String, _>("business_cloud_live_smoke_enabled")?
                .eq_ignore_ascii_case("true");
            let access_token_bound = row.try_get::<bool, _>("access_token_bound")?;
            let app_secret_bound = row.try_get::<bool, _>("app_secret_bound")?;
            let webhook_verify_token_bound =
                row.try_get::<bool, _>("webhook_verify_token_bound")?;
            let webhook_proxy_ready = app_secret_bound && webhook_verify_token_bound;
            let command_submission_ready = access_token_bound
                && business_cloud_live_smoke_enabled
                && runtime.as_deref() == Some("business_cloud_smoke");
            Ok(WhatsAppBusinessCloudProxyAccountState {
                account_id,
                lifecycle_state,
                runtime,
                business_cloud_live_smoke_enabled,
                access_token_bound,
                app_secret_bound,
                webhook_verify_token_bound,
                webhook_proxy_ready,
                command_submission_ready,
                remaining_blockers: business_cloud_proxy_account_blockers(
                    access_token_bound,
                    app_secret_bound,
                    webhook_verify_token_bound,
                    business_cloud_live_smoke_enabled,
...
fn business_cloud_proxy_account_blockers(
    access_token_bound: bool,
    app_secret_bound: bool,
    webhook_verify_token_bound: bool,
    business_cloud_live_smoke_enabled: bool,
    command_submission_ready: bool,
) -> Vec<&'static str> {
    let mut blockers = Vec::new();
    if !app_secret_bound {
        blockers.push("missing_whatsapp_business_cloud_app_secret_binding");
    }
    if !webhook_verify_token_bound {
        blockers.push("missing_whatsapp_business_cloud_webhook_verify_token_binding");
    }
    if !access_token_bound {
        blockers.push("missing_whatsapp_business_cloud_access_token_binding");
    }
    if !business_cloud_live_smoke_enabled {
        blockers.push("business_cloud_live_smoke_not_enabled");
    }
    if !command_submission_ready {
        blockers.push("business_cloud_command_submission_not_ready");
    }
    blockers
}
...
async fn verify_business_cloud_webhook_challenge_token(
    state: &AppState,
    account_id: Option<&str>,
    supplied_token: &str,
) -> Result<String, ApiError> {
    if let Some(account_id) = account_id.map(str::trim).filter(|value| !value.is_empty()) {
        let stored_token = read_business_cloud_webhook_secret(
            state,
            account_id,
            ProviderAccountSecretPurpose::WhatsappBusinessCloudWebhookVerifyToken,
        )
        ...
        return Err(WhatsappWebError::InvalidRequest(
            "Business Cloud webhook verify token mismatch".to_owned(),
        )
        .into());
    }

    for (candidate_account_id, secret_ref) in business_cloud_webhook_secret_refs(
        state,
        ProviderAccountSecretPurpose::WhatsappBusinessCloudWebhookVerifyToken,
    )
    .await?
    {
        match state.vault.read_secret(&secret_ref) {
            Ok(stored_token) if stored_token == supplied_token => return Ok(candidate_account_id),
            Ok(_) => {}
            Err(error) => {
                tracing::warn!(
                    account_id = %candidate_account_id,
                    secret_purpose = ProviderAccountSecretPurpose::WhatsappBusinessCloudWebhookVerifyToken.as_str(),
                    error = %error,
                    "skipping unreadable WhatsApp Business Cloud webhook verify token"
                );
            }
        }
    }
    ...
    Err(WhatsappWebError::InvalidRequest(
        "Business Cloud webhook verify token is not configured or does not match".to_owned(),
    )
    .into())
}

async fn verify_business_cloud_webhook_signature(
    state: &AppState,
    account_id: &str,
    headers: &HeaderMap,
    body: &[u8],
) -> Result<(), ApiError> {
    let app_secret = read_business_cloud_webhook_secret(
        state,
        account_id,
        ProviderAccountSecretPurpose::WhatsappBusinessCloudAppSecret,
    )
    ...
    let signature = headers
        ...
        .ok_or_else(|| {
            WhatsappWebError::InvalidRequest(
                "X-Hub-Signature-256 is required for Business Cloud webhook ingestion".to_owned(),
            )
        })?;
    let signature = signature.strip_prefix("sha256=").ok_or_else(|| {
        WhatsappWebError::InvalidRequest(
            "X-Hub-Signature-256 must use sha256=<hex-digest>".to_owned(),
        )
    })?;
    let signature = decode_sha256_hex(signature).ok_or_else(|| {
        WhatsappWebError::InvalidRequest(
            "X-Hub-Signature-256 must contain a 32-byte hex digest".to_owned(),
        )
    })?;
    let mut mac = HmacSha256::new_from_slice(app_secret.as_bytes()).map_err(|_| {
        WhatsappWebError::InvalidRequest(
            "Business Cloud webhook app secret is invalid for HMAC verification".to_owned(),
        )
    })?;
    mac.update(body);
    mac.verify_slice(&signature).map_err(|_| {
        WhatsappWebError::InvalidRequest(
            "Business Cloud webhook signature verification failed".to_owned(),
        )
    })?;
    Ok(())
}

async fn read_business_cloud_webhook_secret(
    state: &AppState,
    account_id: &str,
    purpose: ProviderAccountSecretPurpose,
) -> Result<String, ApiError> {
    let binding = communication_provider_secret_binding_store(state)?
        .get_for_account(account_id, purpose)
        .await
        .map_err(|error| WhatsappWebError::ProviderAccountStore(error.to_string()))?
        .ok_or_else(|| {
            WhatsappWebError::InvalidRequest(format!(
                "WhatsApp Business Cloud `{}` secret is not configured for account `{}`",
                purpose.as_str(),
                account_id
            ))
        })?;
    state
        ...
        .map_err(WhatsappWebError::HostVault)
        .map_err(Into::into)
}

async fn business_cloud_webhook_secret_refs(
    state: &AppState,
    purpose: ProviderAccountSecretPurpose,
...
fn business_cloud_webhook_values(payload: &Value) -> Vec<&Value> {
            ...
            if let Some(changes) = entry.get("changes").and_then(Value::as_array) {
                for change in changes {
                    if change.get("field").and_then(Value::as_str) == Some("messages")
                        && let Some(value) = change.get("value")
                    {
                        values.push(value);
                    }
                }
    ...
    if values.is_empty() && (payload.get("messages").is_some() || payload.get("statuses").is_some())
    {
        values.push(payload);
    }
    values
...
fn business_cloud_webhook_explicit_account_id(payload: &Value) -> Option<&str> {
    payload
        .get("_hermes")
...
async fn business_cloud_webhook_account_id(
    state: &AppState,
    ...
    if let Some(account_id) = explicit_account_id {
        let account = sqlx::query_scalar::<_, String>(
            ...
            WHERE account_id = $1
              AND provider_kind = 'whatsapp_business_cloud'
              AND COALESCE(config->>'provider_shape', '') = 'whatsapp_business_cloud'
            "#,
        )
        .bind(account_id)
        .fetch_optional(pool)
        .await
        .map_err(WhatsappWebError::Sqlx)?;
        return account.ok_or_else(|| {
            WhatsappWebError::InvalidRequest(format!(
                "WhatsApp Business Cloud account `{account_id}` is not configured"
            ))
            .into()
        });
    }

    let phone_number_id = business_cloud_phone_number_id(webhook_value).ok_or_else(|| {
        WhatsappWebError::InvalidRequest(
            "Business Cloud webhook metadata.phone_number_id is required when _hermes.account_id is absent"
                .to_owned(),
        )
    })?;
    let account = sqlx::query_scalar::<_, String>(
        ...
        SELECT account_id
        FROM communication_provider_accounts
        WHERE provider_kind = 'whatsapp_business_cloud'
          AND COALESCE(config->>'provider_shape', '') = 'whatsapp_business_cloud'
          AND (
                external_account_id = $1
             OR config->>'business_cloud_phone_number_id' = $1
             OR config->>'phone_number_id' = $1
          )
    ...
    account.ok_or_else(|| {
        WhatsappWebError::InvalidRequest(format!(
            "No WhatsApp Business Cloud account is configured for phone_number_id `{phone_number_id}`"
        ))
        .into()
    })
}

fn business_cloud_webhook_message_request(
    account_id: &str,
    webhook_value: &Value,
    message: &Value,
) -> Option<NewWhatsappWebMessage> {
    if message.get("type").and_then(Value::as_str) != Some("text") {
        return None;
    }
    let provider_message_id = required_webhook_str(message, "id")?.to_owned();
    let sender_id = required_webhook_str(message, "from")?.to_owned();
    let text = message
        .get("text")
        ...
        .filter(|value| !value.is_empty())?
        .to_owned();
    let sender_display_name = business_cloud_contact_display_name(webhook_value, &sender_id)
        .unwrap_or_else(|| sender_id.clone());
    let occurred_at = business_cloud_webhook_timestamp(message);
    Some(NewWhatsappWebMessage {
        account_id: account_id.to_owned(),
        provider_chat_id: sender_id.clone(),
        provider_message_id: provider_message_id.clone(),
        chat_title: sender_display_name.clone(),
        sender_id,
        sender_display_name,
        text,
        ...
        message_metadata: json!({
            "provider_shape": "whatsapp_business_cloud",
            "webhook_source": "business_cloud_webhook",
            "message_type": "text",
            "phone_number_id_hash": business_cloud_phone_number_id(webhook_value)
                .map(whatsapp_short_hash),
            "raw_provider_payload": "excluded",
            "api_access_token": "excluded",
            "app_secret": "excluded",
            "webhook_verify_token": "excluded",
            "payload_policy": "sanitized_metadata_only_except_message_text_source_evidence",
        }),
        import_batch_id: business_cloud_import_batch_id(account_id, &provider_message_id),
        occurred_at,
        delivery_state: WhatsappWebDeliveryState::Received,
    })
}

fn business_cloud_webhook_receipt_request(
    account_id: &str,
    status: &Value,
) -> Option<NewWhatsappWebReceipt> {
    let provider_message_id = required_webhook_str(status, "id")?.to_owned();
    let provider_chat_id = required_webhook_str(status, "recipient_id")?.to_owned();
    let delivery_state = business_cloud_status_delivery_state(
        required_webhook_str(status, "status").unwrap_or_default(),
    )?;
    Some(NewWhatsappWebReceipt {
        account_id: account_id.to_owned(),
        provider_chat_id,
        provider_message_id: provider_message_id.clone(),
        delivery_state,
        import_batch_id: business_cloud_import_batch_id(account_id, &provider_message_id),
        observed_at: business_cloud_webhook_timestamp(status),
    })
}

async fn ingest_business_cloud_unsupported_webhook_event(
    ingest_service: &WhatsappFixtureIngestApplicationService,
    account_id: &str,
    webhook_value: &Value,
    item: &Value,
    runtime_event_kind: &str,
) -> Result<String, ApiError> {
    let provider_event_id = business_cloud_webhook_item_id(item).unwrap_or_else(|| {
        format!(
            "business-cloud-webhook:{}",
            whatsapp_short_hash(item.to_string())
        )
    });
    let result = ingest_service
        .ingest_runtime_bridge_runtime_event(&NewWhatsappWebRuntimeEvent {
            account_id: account_id.to_owned(),
            provider_event_id: provider_event_id.clone(),
            runtime_event_kind: runtime_event_kind.to_owned(),
            runtime_status: Some("degraded".to_owned()),
            lifecycle_state: Some("degraded".to_owned()),
            ...
            import_batch_id: business_cloud_import_batch_id(account_id, &provider_event_id),
            observed_at: business_cloud_webhook_timestamp(item),
        })
        .await?;
    Ok(result.raw_record_id)
}

fn business_cloud_status_delivery_state(status: &str) -> Option<WhatsappWebDeliveryState> {
    match status.trim() {
        "sent" => Some(WhatsappWebDeliveryState::Sent),
        "delivered" => Some(WhatsappWebDeliveryState::Delivered),
        "read" => Some(WhatsappWebDeliveryState::Read),
        "played" => Some(WhatsappWebDeliveryState::Played),
        _ => None,
    }
}

fn business_cloud_phone_number_id(webhook_value: &Value) -> Option<&str> {
    webhook_value
        .get("metadata")
        .and_then(|value| value.get("phone_number_id"))
        .and_then(Value::as_str)
        .map(str::trim)
        .filter(|value| !value.is_empty())
}

fn business_cloud_contact_display_name(webhook_value: &Value, wa_id: &str) -> Option<String> {
    webhook_value
        .get("contacts")
        .and_then(Value::as_array)
        .and_then(|contacts| {
            contacts.iter().find(|contact| {
                contact
                    .get("wa_id")
                    .and_then(Value::as_str)
                    .is_some_and(|value| value.trim() == wa_id)
            })
        })
...
fn business_cloud_webhook_timestamp(item: &Value) -> DateTime<Utc> {
    item.get("timestamp")
        .and_then(Value::as_str)
...
fn required_webhook_str<'a>(value: &'a Value, field: &str) -> Option<&'a str> {
    value
        .get(field)
        .and_then(Value::as_str)
        .map(str::trim)
...
fn business_cloud_webhook_item_id(item: &Value) -> Option<String> {
    required_webhook_str(item, "id")
        .or_else(|| required_webhook_str(item, "wamid"))
        .map(str::to_owned)
}

fn business_cloud_import_batch_id(account_id: &str, provider_event_id: &str) -> String {
    format!(
        "whatsapp-business-cloud-webhook:{}:{}",
        account_id.trim(),
        whatsapp_short_hash(provider_event_id)
    )
}

fn whatsapp_short_hash(value: impl AsRef<str>) -> String {
...
async fn ensure_whatsapp_sync_supported(
    ...
    let status = whatsapp_provider_runtime_service(state)?
        ...
        .await?;
    if status.provider_shape == "whatsapp_business_cloud" {
        return Err(WhatsappWebError::InvalidRequest(format!(
            "WhatsApp provider shape `{}` does not support personal sync operation `{operation}`",
            status.provider_shape
        ))
        .into());
    }
...
async fn list_whatsapp_sync_chats(
    ...
    let rows = sqlx::query(
        ...
        WHERE channel_kind IN ('whatsapp_web', 'whatsapp_business_cloud')
          AND account_id = $1
...
async fn list_whatsapp_sync_members(
    ...
    let rows = sqlx::query(
        ...
        WHERE conversation.account_id = $1
          ...
          AND conversation.channel_kind IN ('whatsapp_web', 'whatsapp_business_cloud')
...
async fn list_whatsapp_sync_calls(
    ...
    let rows = sqlx::query(
        ...
        FROM telegram_calls
        WHERE account_id = $1
          AND metadata->>'provider' IN ('whatsapp_web', 'whatsapp_business_cloud')
