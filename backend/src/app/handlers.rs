...
fn graph_store(state: &AppState) -> Result<crate::domains::graph::core::GraphStore, ApiError> {
    let Some(pool) = state.database.pool() else {
        return Err(ApiError::DatabaseNotConfigured);
    };
...
fn ai_run_store(state: &AppState) -> Result<crate::ai::core::AiRunStore, ApiError> {
    let Some(pool) = state.database.pool() else {
        return Err(ApiError::DatabaseNotConfigured);
    };
