CREATE TABLE hermes_data.mail_retained_evidence_replay_index (
    attachment_anchor_id BYTEA PRIMARY KEY CHECK (octet_length(attachment_anchor_id) = 16),
    message_id BYTEA NOT NULL UNIQUE REFERENCES hermes_data.mail_attachment_security_outbox (
        message_id
    ),
...
CREATE TABLE hermes_data.mail_retained_evidence_replay_audit (
...
ON hermes_data.mail_retained_evidence_replay_audit (
    operation_id,
    original_message_id,
    audit_sequence
);
