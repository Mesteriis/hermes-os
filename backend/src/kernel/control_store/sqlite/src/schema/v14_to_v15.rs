...
pub(super) fn apply(transaction: &Transaction<'_>) -> Result<(), StoreError> {
    transaction.execute_batch(
        "CREATE TABLE hermes_kernel_platform_managed_process_binding (
        ...
        CREATE TABLE hermes_kernel_platform_managed_process_launch (
            process_id TEXT PRIMARY KEY REFERENCES hermes_kernel_platform_managed_process_binding(process_id) ON DELETE CASCADE,
            binding_revision INTEGER NOT NULL CHECK (binding_revision >= 1),
        ...
        UPDATE hermes_kernel_control_store_metadata SET schema_version = 15 WHERE singleton = 1;",
    )?;
    Ok(())
}
