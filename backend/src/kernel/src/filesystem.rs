...
pub fn resolve_runtime_directory(data_dir: &Path) -> Result<PathBuf, String> {
    ...
    let instance_key = Sha256::digest(data_dir.as_os_str().as_encoded_bytes())
        .iter().take(16).map(|byte| format!("{byte:02x}")).collect::<String>();
...
pub fn acquire_runtime_directory_lock(runtime_dir: &Path) -> Result<File, String> {
    ...
    let lock = File::options().read(true).write(true).create(true).open(lock_path).map_err(|error| error.to_string())?;
    lock.try_lock().map_err(|_| "data directory is already in use".to_owned())?;
    Ok(lock)
}

pub fn ensure_owner_private_directory(directory: &Path) -> Result<(), String> {
    let mode = std::fs::metadata(directory).map_err(|error| error.to_string())?.permissions().mode();
    if mode & 0o077 != 0 { return Err("data directory must be owner-private".to_owned()); }
    Ok(())
}

pub fn ensure_not_symlink(path: &Path, label: &str) -> Result<(), String> {
    match std::fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() => Err(format!("{label} must not be a symlink")),
        Ok(_) => Ok(()),
...
pub fn prepare_owner_private_directory(directory: &Path) -> Result<(), String> {
    ...
    if existed { ensure_owner_private_directory(directory) } else { std::fs::set_permissions(directory, std::fs::Permissions::from_mode(0o700)).map_err(|error| error.to_string()) }
}
