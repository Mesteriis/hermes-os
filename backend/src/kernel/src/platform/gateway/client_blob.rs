...
pub(crate) fn compose_client_blob_routers(
    ...
    let handler: ClientBlobRouteHandler = Arc::new(move |route, logical_owner_id, payload| {
        adapter.authorize_and_read(route, logical_owner_id, payload)
    });
...
impl KernelClientBlobAdapterV1 {
    ...
    fn authorize_and_read(
        ...
        let (runtime, authorization) =
            self.authorize_module_read(route, logical_owner_id, request_payload)?;
