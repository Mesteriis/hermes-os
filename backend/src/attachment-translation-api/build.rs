...
fn main() {
    ...
    let descriptor = output.join("attachment-translation-v1.bin");
    prost_build::Config::new()
        ...
        .expect("Attachment Translation protocol must compile");
    let digest: [u8; 32] =
        Sha256::digest(std::fs::read(&descriptor).expect("descriptor must exist")).into();
    std::fs::write(
        output.join("attachment_translation_schema.rs"),
        format!("pub const ATTACHMENT_TRANSLATION_SCHEMA_SHA256: [u8; 32] = {digest:?};\n"),
    )
