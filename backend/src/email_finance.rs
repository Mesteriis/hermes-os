...
impl EmailFinanceStore {
    ...
    pub async fn upsert_invoice(
        ...
        invoice.validate()?;
        let row = sqlx::query(
            r#"INSERT INTO email_invoices (invoice_id, message_id, amount, currency, invoice_number, issue_date, due_date, counterparty, tax_id, status, linked_project_id, linked_person_id, metadata)
