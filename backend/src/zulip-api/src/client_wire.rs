...
pub enum ZulipClientWireErrorV1 {
    InvalidPayload,
    MissingVariant,
}
...
fn command_message(command: &ZulipCommandV1) -> wire::ZulipProviderCommandV1 {
    ...
    let command = match command {
        ZulipCommandV1::SendStream {
            ...
            topic,
            content,
        } => Command::SendStream(wire::SendStreamCommandV1 {
            operation_id: operation_id.clone(),
        ...
        ZulipCommandV1::SendDirect {
            ...
            recipients,
            content,
        } => Command::SendDirect(wire::SendDirectCommandV1 {
            operation_id: operation_id.clone(),
            account_id: account_id.clone(),
        ...
        ZulipCommandV1::UpdateMessage {
            ...
            content,
            topic,
        } => Command::UpdateMessage(wire::UpdateMessageCommandV1 {
            operation_id: operation_id.clone(),
            account_id: account_id.clone(),
        ...
        ZulipCommandV1::DeleteMessage {
            ...
            account_id,
            provider_message_id,
        } => Command::DeleteMessage(wire::DeleteMessageCommandV1 {
            operation_id: operation_id.clone(),
            account_id: account_id.clone(),
            provider_message_id: provider_message_id.clone(),
        }),
        ZulipCommandV1::Reaction {
            ...
            reaction,
            operation,
        } => Command::Reaction(wire::ReactionCommandV1 {
            operation_id: operation_id.clone(),
            account_id: account_id.clone(),
            provider_message_id: provider_message_id.clone(),
            ...
            add: *operation == ZulipReactionOperationV1::Add,
        }),
        ZulipCommandV1::SendStreamWithUpload {
            ...
            blob,
            filename,
        } => Command::SendStreamWithUpload(wire::SendStreamWithUploadCommandV1 {
            operation_id: operation_id.clone(),
            account_id: account_id.clone(),
            stream: stream.clone(),
            topic: topic.clone(),
            content: content.clone(),
            blob: Some(blob_message(blob)),
        ...
        ZulipCommandV1::SendDirectWithUpload {
            ...
            blob,
            filename,
        } => Command::SendDirectWithUpload(wire::SendDirectWithUploadCommandV1 {
            operation_id: operation_id.clone(),
            account_id: account_id.clone(),
            recipient: recipients.clone(),
            content: content.clone(),
        ...
        ZulipCommandV1::DownloadAttachment {
            ...
            upload_path,
            blob,
        } => Command::DownloadAttachment(wire::DownloadAttachmentCommandV1 {
            operation_id: operation_id.clone(),
            account_id: account_id.clone(),
    ...
    wire::ZulipProviderCommandV1 {
        command: Some(command),
    }
}

fn decode_command_message(
    message: wire::ZulipProviderCommandV1,
) -> Result<ZulipCommandV1, ZulipClientWireErrorV1> {
    use wire::zulip_provider_command_v1::Command;
    match message
        .command
        .ok_or(ZulipClientWireErrorV1::MissingVariant)?
    {
        Command::SendStream(value) => Ok(ZulipCommandV1::SendStream {
            operation_id: value.operation_id,
            account_id: value.account_id,
            stream: value.stream,
            topic: value.topic,
        ...
        Command::SendDirect(value) => Ok(ZulipCommandV1::SendDirect {
            operation_id: value.operation_id,
            account_id: value.account_id,
            recipients: value.recipient,
            content: value.content,
        }),
        Command::UpdateMessage(value) => Ok(ZulipCommandV1::UpdateMessage {
            operation_id: value.operation_id,
            account_id: value.account_id,
        ...
        Command::DeleteMessage(value) => Ok(ZulipCommandV1::DeleteMessage {
            operation_id: value.operation_id,
            account_id: value.account_id,
            provider_message_id: value.provider_message_id,
        }),
        Command::Reaction(value) => Ok(ZulipCommandV1::Reaction {
            operation_id: value.operation_id,
            account_id: value.account_id,
            provider_message_id: value.provider_message_id,
            ...
            operation: if value.add {
                ZulipReactionOperationV1::Add
            } else {
                ZulipReactionOperationV1::Remove
            },
        }),
        Command::SendStreamWithUpload(value) => Ok(ZulipCommandV1::SendStreamWithUpload {
            operation_id: value.operation_id,
            account_id: value.account_id,
            stream: value.stream,
            topic: value.topic,
        ...
        Command::SendDirectWithUpload(value) => Ok(ZulipCommandV1::SendDirectWithUpload {
            operation_id: value.operation_id,
            account_id: value.account_id,
            recipients: value.recipient,
            content: value.content,
        ...
        Command::DownloadAttachment(value) => Ok(ZulipCommandV1::DownloadAttachment {
            operation_id: value.operation_id,
            account_id: value.account_id,
...
fn decode_blob(
    value: Option<wire::ZulipBlobIntentV1>,
) -> Result<ZulipBlobIntentV1, ZulipClientWireErrorV1> {
...
fn status_message(status: &ZulipCommandOperationStatusV1) -> wire::ZulipCommandOperationStatusV1 {
    let (outcome, provider_message_id, blob_ref) = match &status.outcome {
        ZulipCommandOperationOutcomeV1::OutcomeUnknown => ("outcome_unknown", None, None),
        ZulipCommandOperationOutcomeV1::Accepted {
            provider_message_id,
            blob_ref,
        } => ("accepted", *provider_message_id, blob_ref.clone()),
        ZulipCommandOperationOutcomeV1::Rejected => ("rejected", None, None),
    };
    wire::ZulipCommandOperationStatusV1 {
...
fn decode_status_message(
    status: wire::ZulipCommandOperationStatusV1,
) -> Result<ZulipCommandOperationStatusV1, ZulipClientWireErrorV1> {
    let outcome = match status.outcome.as_str() {
        "outcome_unknown"
            if status.provider_message_id.is_none()
                && status.blob_ref.is_none()
        ...
        "accepted"
            if status.completed_at_unix_seconds.is_some()
                && !(status.provider_message_id.is_some() && status.blob_ref.is_some()) =>
        {
            ZulipCommandOperationOutcomeV1::Accepted {
                provider_message_id: status.provider_message_id,
                blob_ref: status.blob_ref,
            }
        }
        "rejected"
            if status.provider_message_id.is_none()
