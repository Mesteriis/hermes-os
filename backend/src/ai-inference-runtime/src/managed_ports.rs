...
use hermes_ai_contracts::{
    ...
    wire::{AiProviderReplyGenerationRequestV1, AiProviderReplyGenerationResultV1},
};
use hermes_ai_inference_core::AiInferenceExecutionPlanV1;
...
impl AiInferenceExecutionPortsV1 for ManagedAiInferenceExecutionPortsV1<'_> {
    fn materialize_source(
        ...
        let source_reference_id: [u8; 16] = plan
            .source
            .reference_id
            .as_slice()
            .try_into()
            .map_err(|_| AiInferenceSourcePortErrorV1::InvalidReceipt)?;
        let source_sha256: [u8; 32] = plan
            .source
            .sha256
            .as_slice()
            .try_into()
        ...
        let transfer = request_managed_blob_custody_transfer_v2(
            ...
            ManagedBlobCustodyTransferRequestV1 {
                ...
                declared_size: plan.source.declared_bytes,
                receipt_sha256: &source_sha256,
                custody_source_proof: &plan.source.custody_transfer_source_proof,
                evidence_id: &plan.run_id,
                evidence_envelope_sha256: &plan.request_digest,
            },
        )
        ...
        let bytes = BlobDataClient::new(read.data_socket_path)
            .and_then(|client| {
                client.read_range(
                    ...
                    0,
                    plan.source.declared_bytes,
                )
            })
            .map_err(|_| AiInferenceSourcePortErrorV1::Unavailable)?;
        if bytes.len() != usize::try_from(plan.source.declared_bytes).unwrap_or(usize::MAX)
