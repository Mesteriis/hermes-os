...
impl PersonPromiseStore {
    ...
    pub async fn create(
        &self, person_id: &str, description: &str, due_at: Option<DateTime<Utc>>,
        ...
        let row = sqlx::query(
            "INSERT INTO person_promises (person_id, description, due_at)
             ...
             RETURNING id::text, person_id, description, source_message_id, promised_at,
                       due_at, fulfilled_at, status, created_at, updated_at"
        ).bind(person_id).bind(description).bind(due_at).fetch_one(&self.pool).await?;
        row_to_promise(row)
    }
    ...
    pub async fn mark_broken(&self, id: &str) -> Result<(), PersonTrustError> {
        sqlx::query("UPDATE person_promises SET status = 'broken', updated_at = now() WHERE id::text = $1")
            .bind(id).execute(&self.pool).await?;
        Ok(())
    }
}
...
fn row_to_promise(row: PgRow) -> Result<PersonPromise, PersonTrustError> {
    Ok(PersonPromise {
        ...
        description: row.try_get("description")?, source_message_id: row.try_get("source_message_id")?,
        promised_at: row.try_get("promised_at")?, due_at: row.try_get("due_at")?,
        fulfilled_at: row.try_get("fulfilled_at")?, status: row.try_get("status")?,
...
impl PersonRiskStore {
    ...
    pub async fn report(
        &self, person_id: &str, risk_type: &str, description: &str, severity: &str, source: &str,
        ...
        let row = sqlx::query(
            "INSERT INTO person_risks (person_id, risk_type, description, severity, source)
             ...
             RETURNING id::text, person_id, risk_type, description, severity, source, confidence,
                       created_at, resolved_at, resolution"
        ).bind(person_id).bind(risk_type).bind(description).bind(severity).bind(source).fetch_one(&self.pool).await?;
        row_to_risk(row)
    }

    pub async fn resolve(&self, id: &str, resolution: &str) -> Result<(), PersonTrustError> {
        sqlx::query("UPDATE person_risks SET resolved_at = now(), resolution = $2 WHERE id::text = $1")
            .bind(id).bind(resolution).execute(&self.pool).await?;
        Ok(())
    }
}
...
fn row_to_risk(row: PgRow) -> Result<PersonRisk, PersonTrustError> {
    Ok(PersonRisk {
        id: row.try_get("id")?, person_id: row.try_get("person_id")?,
        risk_type: row.try_get("risk_type")?, description: row.try_get("description")?,
        severity: row.try_get("severity")?, source: row.try_get("source")?,
        confidence: row.try_get("confidence")?, created_at: row.try_get("created_at")?,
        resolved_at: row.try_get("resolved_at")?, resolution: row.try_get("resolution")?,
    })
}
