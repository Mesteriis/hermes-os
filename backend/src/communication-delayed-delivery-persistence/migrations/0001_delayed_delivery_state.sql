CREATE TABLE hermes_data.communication_delayed_delivery_operations (
...
CREATE TABLE hermes_data.communication_delayed_delivery_scheduler_inbox (
  logical_owner_id TEXT NOT NULL CHECK (
    char_length(logical_owner_id) BETWEEN 1 AND 128
  ),
...
CREATE TABLE hermes_data.communication_delayed_delivery_outbox (
  logical_owner_id TEXT NOT NULL CHECK (
    char_length(logical_owner_id) BETWEEN 1 AND 128
  ),
  message_id BYTEA NOT NULL CHECK (octet_length(message_id) = 16),
...
CREATE INDEX communication_delayed_delivery_outbox_pending_idx
  ON hermes_data.communication_delayed_delivery_outbox (
    logical_owner_id,
