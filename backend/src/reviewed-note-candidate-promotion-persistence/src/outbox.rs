...
pub(crate) async fn insert_exact_outbox(
    ...
    let inserted = sqlx::query(
        "INSERT INTO hermes_data.reviewed_note_candidate_promotion_outbox (
...
pub(crate) async fn verify_exact_outbox(
    ...
    let row = sqlx::query(
        "SELECT envelope_sha256, envelope_bytes
         FROM hermes_data.reviewed_note_candidate_promotion_outbox
...
impl ReviewedNoteCandidatePromotionPersistenceV1 {
    ...
    pub async fn mark_event_published(
        ...
        let updated = sqlx::query(
            "UPDATE hermes_data.reviewed_note_candidate_promotion_outbox
