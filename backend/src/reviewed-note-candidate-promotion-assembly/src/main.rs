...
use hermes_reviewed_note_candidate_promotion_assembly::materialize_reviewed_note_candidate_promotion_release_assembly_v1;
...
fn main() -> ExitCode {
    let Some(arguments) = arguments(std::env::args().skip(1).collect()) else {
        return fail(
            "usage: hermes-reviewed-note-candidate-promotion-assembly --build-id <id> \
