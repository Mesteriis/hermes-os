...
impl LegalDocumentStore {
    ...
    pub async fn upsert(
        ...
        doc.validate()?;
        let row = sqlx::query(
            r#"INSERT INTO email_legal_documents (document_id, message_id, document_type, title, parties, effective_date, expiry_date, amount, currency, status, linked_project_id, risks, metadata)
    ...
    pub async fn list(
        ...
        let rows = sqlx::query(
            ...
            FROM email_legal_documents WHERE ($1::text IS NULL OR document_type = $1) AND ($2::text IS NULL OR status = $2) ORDER BY COALESCE(effective_date, created_at) DESC"#,
