...
impl ReviewNoteCandidatePersistenceV1 {
    ...
    ) -> Result<ReserveReviewNoteCandidateSubmissionOutcomeV1, ReviewNoteCandidatePersistenceErrorV1>
    {
        ...
        let inserted = sqlx::query(
            "INSERT INTO hermes_data.review_note_candidate_submissions (
    ...
    ) -> Result<PersistedReviewNoteCandidateSubmissionV1, ReviewNoteCandidatePersistenceErrorV1>
    {
        ...
        let affected = sqlx::query(
            "UPDATE hermes_data.review_note_candidate_submissions
    ...
    pub async fn decide(
        ...
        if let Some(row) = sqlx::query(
            "SELECT request_sha256, decision_fingerprint, review_id, result_review_revision
             FROM hermes_data.review_note_candidate_operations
        ...
        sqlx::query(
            "INSERT INTO hermes_data.review_note_candidate_operations (
    ...
    pub async fn persist_promotion_result(
        ...
        if let Some(row) = sqlx::query(
            "SELECT result_envelope_sha256, review_id
             FROM hermes_data.review_note_candidate_promotion_inbox
        ...
        update_review(&mut transaction, current.review_revision, &next).await?;
        sqlx::query(
            "INSERT INTO hermes_data.review_note_candidate_promotion_inbox (
    ...
    ) -> Result<Vec<ReviewNoteCandidateRealtimeTransitionV1>, ReviewNoteCandidatePersistenceErrorV1>
    {
        ...
        sqlx::query(
            "SELECT realtime_sequence, review_id, candidate_id, state,
             ...
             FROM hermes_data.review_note_candidate_realtime
    ...
    pub async fn mark_outbox_published(
        ...
        let affected = sqlx::query(
            "UPDATE hermes_data.review_note_candidate_outbox
...
async fn insert_outbox(
    ...
    sqlx::query(
        "INSERT INTO hermes_data.review_note_candidate_outbox (
...
async fn insert_realtime(
    ...
    sqlx::query(
        "INSERT INTO hermes_data.review_note_candidate_realtime (
...
fn transition_error(
    error: hermes_review_note_candidate_core::ReviewNoteCandidateTransitionErrorV1,
) -> ReviewNoteCandidatePersistenceErrorV1 {
    match error {
        hermes_review_note_candidate_core::ReviewNoteCandidateTransitionErrorV1::RevisionConflict => {
