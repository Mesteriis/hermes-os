...
pub(crate) async fn post_v1_translate_attachment(
    ...
    let detection =
        crate::domains::mail::multilingual::MultilingualService::detect_language(source_text);
    let service = email_multilingual_service(&state).await?;
...
pub(crate) async fn post_v1_translate_thread(
    ...
    let pool = state
        ...
        .clone();
    let thread_store = crate::domains::mail::threads::EmailThreadStore::new(pool);
    ...
    for message in messages {
        let detection = crate::domains::mail::multilingual::MultilingualService::detect_language(
            &message.body_text,
        );
