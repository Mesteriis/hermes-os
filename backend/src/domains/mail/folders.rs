...
impl MailFolderStore {
    ...
    pub async fn list(
        ...
        let rows = sqlx::query(
            ...
            SELECT
                ...
                f.updated_at
            FROM mail_folders f
            LEFT JOIN mail_folder_messages fm ON fm.folder_id = f.folder_id
    ...
    async fn apply_message_action_with_observation(
        ...
        if matches!(operation, FolderMessageOperation::Move) {
            sqlx::query(
                r#"
                DELETE FROM mail_folder_messages
...
async fn delete_folder(
    ...
    let row = sqlx::query(
        ...
        deleted AS (
            DELETE FROM mail_folders WHERE folder_id = $1 RETURNING folder_id
        )
        SELECT target.*
        FROM target
...
async fn folder_exists(
    ...
    Ok(sqlx::query_scalar::<_, bool>(
        "SELECT EXISTS (SELECT 1 FROM mail_folders WHERE folder_id = $1)",
    )
    .bind(folder_id)
    .fetch_one(&mut **transaction)
    .await?)
...
async fn upsert_folder_message(
    ...
    sqlx::query(
        r#"
        INSERT INTO mail_folder_messages (folder_id, message_id, added_at, last_operation)
        VALUES ($1, $2, now(), $3)
