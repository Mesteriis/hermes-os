...
use super::background_sync::DEFAULT_MAIL_SYNC_BLOB_ROOT;
use super::core::ProviderAccount;
use super::drafts::{DraftStatus, EmailDraft, EmailDraftError, EmailDraftStore, NewEmailDraft};
...
use super::outbox::{
    EmailOutboxError, EmailOutboxItem, EmailOutboxStatus, EmailOutboxStore, NewEmailOutboxItem,
};
...
use super::send::{OutgoingEmail, ProviderSendStore, ProviderSendStoreError};
use super::storage::{
...
impl MailCommandService {
    ...
    pub async fn upsert_draft(
    ...
    ) -> Result<EmailDraft, MailCommandServiceError> {
        ...
        let store = EmailDraftStore::new(self.pool.clone());
    ...
    pub async fn undo_outbox(
        &self,
        outbox_id: &str,
    ) -> Result<EmailOutboxItem, MailCommandServiceError> {
        let observation = self
            .capture_observation(
                "outbox undo",
        ...
        Ok(EmailOutboxStore::new(self.pool.clone())
    ...
    pub async fn enqueue_outbox_send(
        ...
        let status = match command.scheduled_send_at {
            Some(scheduled_send_at) if scheduled_send_at > now => EmailOutboxStatus::Scheduled,
            _ => EmailOutboxStatus::Queued,
        };
        let operation = if status == EmailOutboxStatus::Scheduled {
            "outbox_schedule"
        } else {
            "outbox_enqueue"
        };
