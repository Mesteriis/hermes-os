...
pub(super) fn normalize_email_address(
    ...
    if normalized_email.is_empty() {
        return Err(PersonaProjectionError::EmptyEmailAddress);
    }
    if !normalized_email.contains('@') {
        return Err(PersonaProjectionError::InvalidEmailAddress(
            normalized_email,
        ));
    }
...
pub(super) fn persona_id_for_email(normalized_email: &str) -> String {
...
pub(super) fn normalize_ai_agent_id(agent_id: &str) -> Result<String, PersonaProjectionError> {
    let normalized = agent_id.trim().to_ascii_uppercase();
    if normalized.is_empty() {
        return Err(PersonaProjectionError::EmptyAiAgentId);
    }
    if !normalized
        .chars()
        .all(|ch| ch.is_ascii_alphanumeric() || ch == '_' || ch == '-')
    {
        return Err(PersonaProjectionError::InvalidAiAgentId(
            agent_id.to_owned(),
        ));
    }
    Ok(normalized)
...
pub(super) fn validate_display_name(display_name: &str) -> Result<String, PersonaProjectionError> {
...
pub(super) fn ai_agent_persona_id(agent_id: &str) -> String {
