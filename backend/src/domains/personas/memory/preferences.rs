...
impl PersonaPreferenceStore {
    ...
    pub async fn list(
        ...
        let rows = sqlx::query(
            "SELECT id::text, persona_id, preference_type, value, source, confidence::float8 AS confidence,
             last_verified_at, created_at, updated_at FROM persona_preferences
             WHERE persona_id = $1 ORDER BY preference_type",
        )
        .bind(persona_id)
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn upsert(
        ...
        let row = sqlx::query(
            "INSERT INTO persona_preferences (persona_id, preference_type, value, source)
             VALUES ($1, $2, $3, $4)
             ON CONFLICT (persona_id, preference_type) DO UPDATE SET value = $3, source = $4, updated_at = now()
             RETURNING id::text, persona_id, preference_type, value, source, confidence::float8 AS confidence,
                       last_verified_at, created_at, updated_at"
        ).bind(persona_id).bind(preference_type).bind(value).bind(source).fetch_one(&self.pool).await?;
    ...
    pub async fn upsert_with_observation(
        ...
        let pref = self
            .upsert(persona_id, preference_type, value, source)
            .await?;
        link_persona_entity(
            &self.pool,
            observation_id,
