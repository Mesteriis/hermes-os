...
) -> Result<String, CommunicationThreadError> {
    let cursor = ThreadListCursor {
        last_activity_at: thread.last_activity_at,
        thread_id: thread.thread_id.clone(),
    };
    let bytes = serde_json::to_vec(&cursor).map_err(|_| CommunicationThreadError::InvalidCursor)?;

    Ok(URL_SAFE_NO_PAD.encode(bytes))
}

fn decode_thread_list_cursor(cursor: &str) -> Result<ThreadListCursor, CommunicationThreadError> {
    let bytes = URL_SAFE_NO_PAD
        .decode(cursor)
        .map_err(|_| CommunicationThreadError::InvalidCursor)?;
    let cursor: ThreadListCursor =
        serde_json::from_slice(&bytes).map_err(|_| CommunicationThreadError::InvalidCursor)?;
    if cursor.thread_id.trim().is_empty() {
        return Err(CommunicationThreadError::InvalidCursor);
    }
...
#[derive(Debug, Error)]
pub enum CommunicationThreadError {
    #[error(transparent)]
