//! Bounded framing for the private inherited Scheduler control channel.
...
pub(super) fn read_frame(stream: &mut UnixStream) -> Result<Vec<u8>, String> {
    let length = usize::try_from(read_varint(stream)?)
        .map_err(|_| "Scheduler inherited control frame is invalid".to_owned())?;
    if length == 0 || length > MAX_FRAME_BYTES {
        return Err("Scheduler inherited control frame is invalid".to_owned());
    }
...
fn read_varint(stream: &mut impl Read) -> Result<u64, String> {
    ...
    Err("Scheduler inherited control frame is invalid".to_owned())
}
