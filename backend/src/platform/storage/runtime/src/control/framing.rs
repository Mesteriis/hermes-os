//! Bounded binary framing shared by the Storage inherited-control client.
...
pub(super) fn read_frame(stream: &mut UnixStream) -> Result<Vec<u8>, String> {
    ...
    if length == 0 || length > MAX_FRAME_BYTES {
        return Err("Storage inherited control frame is invalid".to_owned());
    }
...
fn read_varint(stream: &mut impl Read) -> Result<u64, String> {
    ...
    Err("Storage inherited control frame is invalid".to_owned())
}
