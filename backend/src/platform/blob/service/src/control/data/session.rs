...
use hermes_runtime_protocol::v1::{
    BlobBackupClassV1 as WireBackupClass, BlobCustodyTransferGrantV1, BlobDataOperationV1, BlobDataSessionGrantV1,
};
...
impl VerifiedBlobCustodyTransferV1 {
    pub(super) fn source_reference(&self) -> &BlobRefV1 { &self.source_reference }
    pub(super) fn source_access(&self) -> &BlobAccessFenceV1 { &self.source_access }
    pub(super) const fn source_key_revision(&self) -> u64 { self.source_key_revision }
    pub(super) fn target_reference(&self) -> &BlobRefV1 { &self.target_reference }
    pub(super) fn target_access(&self) -> &BlobAccessFenceV1 { &self.target_access }
    pub(super) fn target_quota(&self) -> &BlobQuotaGrantV1 { &self.target_quota }
    pub(super) const fn target_key_revision(&self) -> u64 { self.target_key_revision }
    pub(super) const fn expected_plaintext_sha256(&self) -> &[u8; 32] { &self.expected_plaintext_sha256 }
}
...
impl BlobDataSessionVerifierV1 {
    ...
    pub(super) fn verify(
        ...
        let verified = decode_grant(&grant)?;
...
fn validate_signed_transfer(
    ...
    let signature = Signature::from_slice(&grant.kernel_authorization_signature_raw).map_err(|_| ())?;
...
fn validate_signed_grant(
    ...
    if grant.major != 1
        ...
        || grant.expires_at_unix_ms <= now
        || grant.expires_at_unix_ms > now.checked_add(SESSION_TTL_LIMIT_MS).ok_or(())?
        || BlobDataOperationV1::try_from(grant.operation).ok() != Some(expected)
        || grant.kernel_authorization_signature_raw.len() != 64
    {
        return Err(());
    }
    let actual_binding = Sha256::digest(binding);
    if actual_binding.as_slice() != grant.channel_binding_sha256.as_slice() {
        return Err(());
    }
    let signature =
...
fn decode_grant(grant: &BlobDataSessionGrantV1) -> Result<VerifiedBlobDataSessionV1, ()> {
    let reference_id = grant.reference_id.as_slice().try_into().map_err(|_| ())?;
    let backup_class = match WireBackupClass::try_from(grant.backup_class).map_err(|_| ())? {
        ...
        WireBackupClass::BlobBackupClassUnspecifiedV1 => return Err(()),
    };
...
fn decode_transfer(grant: &BlobCustodyTransferGrantV1) -> Result<VerifiedBlobCustodyTransferV1, ()> {
    let source = grant.source.as_ref().ok_or(())?;
    let backup_class = backup_class(source.backup_class)?;
    let source_reference = BlobRefV1::new(
        source.reference_id.as_slice().try_into().map_err(|_| ())?, source.owner_id.clone(), source.declared_size,
        (source.reference_expires_at_unix_ms != 0).then_some(source.reference_expires_at_unix_ms), backup_class,
    ).map_err(|_| ())?;
    let source_access = BlobAccessFenceV1::new(
        source.owner_id.clone(), source.registration_id.clone(), source.capability_id.clone(),
        source.runtime_instance_id.clone(), source.runtime_generation, source.grant_epoch,
    ).map_err(|_| ())?;
    let target_reference = BlobRefV1::new(
        grant.target_reference_id.as_slice().try_into().map_err(|_| ())?, grant.target_owner_id.clone(), source.declared_size,
        (source.reference_expires_at_unix_ms != 0).then_some(source.reference_expires_at_unix_ms), backup_class,
    ).map_err(|_| ())?;
    let target_access = BlobAccessFenceV1::new(
        ...
        grant.target_runtime_instance_id.clone(), grant.target_runtime_generation, grant.target_grant_epoch,
    ).map_err(|_| ())?;
    ...
    Ok(VerifiedBlobCustodyTransferV1 {
        source_reference, source_access, source_key_revision: source.key_revision,
        target_reference, target_access, target_quota, target_key_revision: grant.target_key_revision,
        expected_plaintext_sha256: source.receipt_sha256.as_slice().try_into().map_err(|_| ())?,
    })
}
