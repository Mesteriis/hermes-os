//! Bounded Protobuf framing for the inherited authority channel.
...
pub(crate) fn read_frame(stream: &mut UnixStream) -> Result<Vec<u8>, String> {
    let length = usize::try_from(read_varint(stream)?)
        .map_err(|_| "Events authority inherited frame is invalid".to_owned())?;
    if length == 0 || length > MAX_FRAME_BYTES {
        return Err("Events authority inherited frame is invalid".to_owned());
    }
...
pub(crate) fn write_frame(stream: &mut UnixStream, bytes: &[u8]) -> Result<(), String> {
...
fn read_varint(stream: &mut impl Read) -> Result<u64, String> {
    ...
    Err("Events authority inherited frame is invalid".to_owned())
}
