CREATE TABLE hermes_data.attachment_security_text_extraction_delegation_inbox (
...
CREATE TABLE hermes_data.attachment_security_text_extraction_delegation_outbox (
  message_id BYTEA PRIMARY KEY CHECK (octet_length(message_id) = 16),
  envelope_sha256 BYTEA NOT NULL CHECK (octet_length(envelope_sha256) = 32),
  exact_envelope_bytes BYTEA NOT NULL CHECK (octet_length(exact_envelope_bytes) > 0),
...
CREATE TABLE hermes_data.attachment_security_text_extraction_delegation_jobs (
  request_message_id BYTEA PRIMARY KEY REFERENCES
    hermes_data.attachment_security_text_extraction_delegation_inbox (message_id),
  request_id BYTEA NOT NULL UNIQUE CHECK (octet_length(request_id) = 16),
  ...
  result_message_id BYTEA UNIQUE REFERENCES
    hermes_data.attachment_security_text_extraction_delegation_outbox (message_id),
...
CREATE INDEX attachment_security_text_extraction_delegation_jobs_pending_idx
  ON hermes_data.attachment_security_text_extraction_delegation_jobs (
...
CREATE INDEX attachment_security_text_extraction_delegation_outbox_pending_idx
  ON hermes_data.attachment_security_text_extraction_delegation_outbox (
