...
CREATE TABLE IF NOT EXISTS hermes_data.whatsapp_delivery_intent_inbox (
...
CREATE TABLE IF NOT EXISTS hermes_data.whatsapp_delivery_intent_jobs (
...
CREATE INDEX IF NOT EXISTS whatsapp_delivery_intent_jobs_claim_idx
    ON hermes_data.whatsapp_delivery_intent_jobs
        (state, next_attempt_at_unix_seconds, intent_id);

CREATE TABLE IF NOT EXISTS hermes_data.whatsapp_delivery_intent_result_outbox (
    message_id BYTEA PRIMARY KEY,
    envelope_sha256 BYTEA NOT NULL,
...
impl WhatsAppDeliveryIntentStoreV1 {
    ...
    pub async fn accept_command(
        ...
        let inserted = sqlx::query(
            "INSERT INTO hermes_data.whatsapp_delivery_intent_inbox
        ...
        if inserted.rows_affected() == 0 {
            let row = sqlx::query(
                "SELECT envelope_sha256, intent_id, logical_owner_id, state
                 FROM hermes_data.whatsapp_delivery_intent_inbox
                 WHERE message_id = $1",
            )
            .bind(admission.command_message_id.as_slice())
        ...
        let outcome = if let Some(route) = route {
            sqlx::query(
                "INSERT INTO hermes_data.whatsapp_delivery_intent_jobs
            ...
            .map_err(|_| WhatsAppDurablePersistenceError::Database)?;
            sqlx::query(
                "UPDATE hermes_data.whatsapp_delivery_intent_inbox SET state = 1
            ...
            .await?;
            sqlx::query(
                "UPDATE hermes_data.whatsapp_delivery_intent_inbox SET state = 2
                 WHERE message_id = $1 AND state = 0",
            )
            .bind(admission.command_message_id.as_slice())
            .execute(&mut *transaction)
    ...
    pub async fn claim_next_job(
        ...
        let row = sqlx::query(
            "WITH next AS (
                SELECT intent_id
                FROM hermes_data.whatsapp_delivery_intent_jobs
             ...
             UPDATE hermes_data.whatsapp_delivery_intent_jobs job
             SET claimed_by = $3,
                 ...
                 END
             FROM next, hermes_data.whatsapp_delivery_intent_inbox inbox
    ...
    pub async fn record_target_body_receipt(
        ...
        let updated = sqlx::query(
            "UPDATE hermes_data.whatsapp_delivery_intent_jobs
             SET state = $1,
    ...
    let row = if let Some(reply_cursor) = admission.reply_to_source_cursor {
        sqlx::query(
            "SELECT account.account_id, conversation.provider_chat_id,
             ...
             FROM hermes_data.whatsapp_delivery_route_accounts account
             JOIN hermes_data.whatsapp_delivery_route_conversations conversation
               ON conversation.account_cursor = account.account_cursor
             JOIN hermes_data.whatsapp_delivery_route_messages message
