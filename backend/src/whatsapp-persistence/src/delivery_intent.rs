...
CREATE TABLE IF NOT EXISTS hermes_data.whatsapp_delivery_route_accounts (
...
CREATE TABLE IF NOT EXISTS hermes_data.whatsapp_delivery_route_conversations (
...
CREATE TABLE IF NOT EXISTS hermes_data.whatsapp_delivery_route_messages (
...
pub(crate) async fn upsert_delivery_route_locator(
    ...
    let account = sqlx::query(
        "INSERT INTO hermes_data.whatsapp_delivery_route_accounts
         ...
         ON CONFLICT (account_cursor) DO UPDATE SET
            ...
            updated_at_unix_seconds = GREATEST(
                hermes_data.whatsapp_delivery_route_accounts.updated_at_unix_seconds,
                EXCLUDED.updated_at_unix_seconds)
         WHERE hermes_data.whatsapp_delivery_route_accounts.account_id = EXCLUDED.account_id",
    )
    ...
    let conversation = sqlx::query(
        "INSERT INTO hermes_data.whatsapp_delivery_route_conversations
            (conversation_cursor, account_cursor, account_id, provider_chat_id,
             updated_at_unix_seconds)
         VALUES ($1, $2, $3, $4, $5)
         ON CONFLICT (conversation_cursor) DO UPDATE SET
            updated_at_unix_seconds = GREATEST(
                hermes_data.whatsapp_delivery_route_conversations.updated_at_unix_seconds,
                EXCLUDED.updated_at_unix_seconds)
         WHERE hermes_data.whatsapp_delivery_route_conversations.account_cursor =
                   EXCLUDED.account_cursor
           AND hermes_data.whatsapp_delivery_route_conversations.account_id =
                   EXCLUDED.account_id
           AND hermes_data.whatsapp_delivery_route_conversations.provider_chat_id =
    ...
    let message = sqlx::query(
        "INSERT INTO hermes_data.whatsapp_delivery_route_messages
         ...
         ON CONFLICT (source_cursor) DO UPDATE SET
            updated_at_unix_seconds = GREATEST(
                hermes_data.whatsapp_delivery_route_messages.updated_at_unix_seconds,
                EXCLUDED.updated_at_unix_seconds)
         WHERE hermes_data.whatsapp_delivery_route_messages.account_cursor =
                   EXCLUDED.account_cursor
           AND hermes_data.whatsapp_delivery_route_messages.conversation_cursor =
                   EXCLUDED.conversation_cursor
           AND hermes_data.whatsapp_delivery_route_messages.account_id = EXCLUDED.account_id
           AND hermes_data.whatsapp_delivery_route_messages.provider_chat_id =
                   EXCLUDED.provider_chat_id
           AND hermes_data.whatsapp_delivery_route_messages.provider_message_id =
