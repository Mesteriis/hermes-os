...
impl AttachmentTextExtractionPersistenceV1 {
    pub async fn store_custody_delegation_outbox(
        ...
        let inserted = sqlx::query(
            "INSERT INTO hermes_data.attachment_text_extraction_custody_outbox (logical_owner_id,request_id,run_id,candidate_message_id,safety_message_id,envelope_sha256,exact_envelope_bytes,published_at_unix_millis,created_at_unix_millis) VALUES ($1,$2,$3,$4,$5,$6,$7,NULL,$8) ON CONFLICT (logical_owner_id,run_id) DO NOTHING",
        )
        .bind(logical_owner_id)
        ...
        if inserted.rows_affected() == 0 {
            let existing = sqlx::query("SELECT request_id,envelope_sha256,exact_envelope_bytes FROM hermes_data.attachment_text_extraction_custody_outbox WHERE logical_owner_id=$1 AND run_id=$2")
    ...
    > {
        ...
        sqlx::query("SELECT request_id,envelope_sha256,exact_envelope_bytes FROM hermes_data.attachment_text_extraction_custody_outbox WHERE logical_owner_id=$1 AND published_at_unix_millis IS NULL ORDER BY created_at_unix_millis,request_id LIMIT $2")
    ...
    pub async fn mark_custody_delegation_published(
        ...
        let changed = sqlx::query("UPDATE hermes_data.attachment_text_extraction_custody_outbox SET published_at_unix_millis=$1 WHERE logical_owner_id=$2 AND request_id=$3 AND envelope_sha256=$4 AND published_at_unix_millis IS NULL AND created_at_unix_millis <= $1")
    ...
    > {
        ...
        let work = AttachmentTextDelegatedWorkV1 {
            request: hermes_attachment_text_extraction_core::AttachmentTextExtractionRequestV1 {
                run_id: request.run_id,
...
async fn insert_result_inbox(
    ...
    let inserted = sqlx::query(
        "INSERT INTO hermes_data.attachment_text_extraction_custody_result_inbox (logical_owner_id,message_id,envelope_sha256,request_id,run_id,attachment_anchor_id,result_kind,processed_at_unix_millis) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) ON CONFLICT DO NOTHING",
    )
    ...
    let row = sqlx::query(
        "SELECT message_id,envelope_sha256,request_id,run_id,attachment_anchor_id,result_kind FROM hermes_data.attachment_text_extraction_custody_result_inbox WHERE logical_owner_id=$1 AND (message_id=$2 OR request_id=$3) FOR UPDATE",
    )
    .bind(fact.logical_owner_id)
    .bind(fact.message_id.as_slice())
    .bind(fact.request_id.as_slice())
