...
/// GET /api/v1/telegram/search/messages?q=&account_id=&provider_chat_id=&limit=
...
/// GET /api/v1/telegram/chats/search?q=&account_id=&limit=
...
/// GET /api/v1/telegram/chats/{telegram_chat_id}/pinned-messages?limit=
pub(crate) async fn get_telegram_pinned_messages(
...
) -> Result<Json<crate::domains::api_support::TelegramMessageListResponse>, ApiError> {
    let store = telegram_store(&state)?;
    let items = store
    ...
    Ok(Json(
        crate::domains::api_support::TelegramMessageListResponse { items },
    ))
}

/// GET /api/v1/telegram/search/media?account_id=&provider_chat_id=&kind=&limit=
