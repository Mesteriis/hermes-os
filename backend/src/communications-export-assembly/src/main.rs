...
use hermes_communications_export_assembly::materialize_communications_export_release_assembly_v1;
...
fn main() -> ExitCode {
    match arguments(std::env::args().skip(1).collect()) {
        ...
        None => fail(
            "usage: hermes-communications-export-assembly --build-id <id> \
...
mod tests {
    ...
    fn parses_exact_options() {
        let parsed = arguments(vec![
            "--runtime".to_owned(),
            "/tmp/hermes-communications-export-runtime".to_owned(),
            "--build-id".to_owned(),
            "build-1".to_owned(),
            "--output-dir".to_owned(),
        ...
        assert_eq!(
            parsed.runtime,
            PathBuf::from("/tmp/hermes-communications-export-runtime")
        );
    }
