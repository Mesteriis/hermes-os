//! WhatsApp managed-runtime identity and opaque Kernel/Vault control channel.
...
pub struct WhatsAppManagedRuntimeIdentity {
...
impl WhatsAppManagedRuntimeIdentity {
    pub fn open_inherited(
        ...
        if duplicated < 0 {
            return Err("WhatsApp managed-runtime channel is unavailable".to_owned());
        }
    ...
    pub fn authenticate(
        ...
        if descriptor_bytes.is_empty() || settings_schema_bytes.is_empty() {
            return Err("WhatsApp managed-runtime descriptor is empty".to_owned());
        }
        let runtime_instance_id = runtime_instance_id.into();
        if runtime_instance_id.trim().is_empty() {
            return Err("WhatsApp managed-runtime instance id is empty".to_owned());
        }
        channel
            .set_read_timeout(Some(CONTROL_TIMEOUT))
            .and_then(|_| channel.set_write_timeout(Some(CONTROL_TIMEOUT)))
            .map_err(|_| "WhatsApp managed-runtime channel is unavailable".to_owned())?;
        write_frame(
            &mut channel,
        ...
        let response = ManagedRuntimeControlResponseV1::decode(read_frame(&mut channel)?.as_slice())
            .map_err(|_| "WhatsApp managed-runtime describe response is invalid".to_owned())?;
        let (registration_id, runtime_generation, grant_epoch) = match response.result {
            ...
            _ => return Err("WhatsApp managed-runtime descriptor was rejected".to_owned()),
        };
        write_frame(
            &mut channel,
            &ManagedRuntimeControlRequestV1 {
    ...
    #[must_use]
    pub fn registration_id(&self) -> &str { &self.registration_id }

    #[must_use]
    pub fn runtime_instance_id(&self) -> &str { &self.runtime_instance_id }

    #[must_use]
    pub const fn runtime_generation(&self) -> u64 { self.runtime_generation }

    #[must_use]
    pub const fn grant_epoch(&self) -> u64 { self.grant_epoch }
}
...
pub fn route_vault_ciphertext(
    ...
    let response = ManagedRuntimeControlResponseV1::decode(read_frame(channel)?.as_slice())
        .map_err(|_| "WhatsApp managed-runtime Vault response is invalid".to_owned())?;
    response
        .response
        .filter(|_| response.error_code.is_empty())
        .ok_or_else(|| "WhatsApp managed-runtime Vault route was denied".to_owned())
}

fn read_frame(channel: &mut UnixStream) -> Result<Vec<u8>, String> {
    let length = usize::try_from(read_varint(channel)?)
        .map_err(|_| "WhatsApp managed-runtime frame is invalid".to_owned())?;
    if length == 0 || length > MAX_FRAME_BYTES {
        return Err("WhatsApp managed-runtime frame is invalid".to_owned());
    }
...
fn write_frame(channel: &mut UnixStream, bytes: Vec<u8>) -> Result<(), String> {
...
fn read_varint(channel: &mut UnixStream) -> Result<u64, String> {
    ...
    Err("WhatsApp managed-runtime frame is invalid".to_owned())
}
