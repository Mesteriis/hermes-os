CREATE TABLE hermes_data.attachment_preview_evidence_replay_anchor_producers (
...
CREATE TABLE hermes_data.attachment_preview_evidence_replay_anchor_result_messages (
    ...
    FOREIGN KEY (operation_id, producer) REFERENCES
        hermes_data.attachment_preview_evidence_replay_anchor_producers (operation_id, producer)
);

CREATE TABLE hermes_data.attachment_preview_evidence_replay_anchor_command_outbox (
...
ON hermes_data.attachment_preview_evidence_replay_anchor_command_outbox (
    created_at_unix_seconds,
    message_id
)
WHERE published_at_unix_seconds IS NULL;

CREATE TABLE hermes_data.attachment_preview_evidence_replay_anchor_result_inbox (
    message_id BYTEA PRIMARY KEY CHECK (octet_length(message_id) = 16),
    envelope_sha256 BYTEA NOT NULL CHECK (octet_length(envelope_sha256) = 32),
    operation_id BYTEA NOT NULL,
    producer SMALLINT NOT NULL,
