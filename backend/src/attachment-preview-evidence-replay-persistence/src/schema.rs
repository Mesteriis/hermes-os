...
pub const ATTACHMENT_PREVIEW_EVIDENCE_REPLAY_STORAGE_BUNDLE_REVISION_V1: u32 = 1;
...
mod tests {
    ...
    fn schema_is_owner_local_metadata_only_and_has_exact_inbox_outbox() {
        validate_storage_bundle(&attachment_preview_evidence_replay_storage_bundle_v1())
            .expect("bundle");
        let sql = std::str::from_utf8(ATTACHMENT_PREVIEW_EVIDENCE_REPLAY_SCHEMA_V1).expect("utf8");
        for required in [
            "attachment_preview_evidence_replay_operations",
            "attachment_preview_evidence_replay_producers",
            "attachment_preview_evidence_replay_message_selection",
            "attachment_preview_evidence_replay_command_outbox",
            "attachment_preview_evidence_replay_result_inbox",
