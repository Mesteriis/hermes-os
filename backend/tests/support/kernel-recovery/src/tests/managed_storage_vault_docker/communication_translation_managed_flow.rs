...
use hermes_communication_translation_runtime::COMMUNICATION_TRANSLATION_STORAGE_CAPABILITY_ID_V1;
...
type CommunicationTranslationGateway = hermes_gateway_runtime::GatewayApplicationRouter<
...
fn managed_communication_translation_reaches_ai_and_replays_through_gateway_sse() {
    ...
    let root = unique_target_root("hermes-managed-communication-translation");
...
fn managed_communication_translation_completes_real_provider_through_gateway_sse() {
    ...
    let root = unique_target_root("hermes-managed-communication-translation-live");
    let data = private_directory(short_communications_kernel_data_directory());
    initialize_vault(
        &private_directory(data.join("vault")),
        &credential_directory(),
