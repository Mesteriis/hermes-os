...
use hermes_mail_api::{
    ...
    MailSendMailRequestV1, client_contract::MailClientContractV1,
};
...
fn managed_mail_runtime_accepts_then_completes_smtp_delivery_and_replays_event() {
    ...
    assert_delivery_completed(&store, &supervisor, &mail);
    ...
    loop {
        ...
        match status.outcome {
            MailDeliveryOutcomeV1::Accepted => {
                assert_eq!(status.response_code, Some(250));
                assert!(status.completed_at_unix_seconds.is_some());
                return;
            }
            MailDeliveryOutcomeV1::Rejected | MailDeliveryOutcomeV1::OutcomeUnknown => {
                panic!("Mail SMTP delivery did not complete successfully")
            }
            MailDeliveryOutcomeV1::Pending => {}
        }
        assert!(
            Instant::now() < deadline,
            "Mail SMTP delivery did not reach a terminal result"
        );
        std::thread::sleep(Duration::from_millis(25));
    }
}

fn route_mail_client(
    store: &SqliteControlStore,
    supervisor: &ManagedRuntimeSupervisor,
    mail: &StartedMailRuntime,
...
fn wait_for_mail_ready(supervisor: &ManagedRuntimeSupervisor, mail: &StartedMailRuntime) {
    let deadline = Instant::now() + Duration::from_secs(10);
