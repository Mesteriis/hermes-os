...
async fn fixture_account_allows_topic_list_but_blocks_topic_writes() {
    ...
    let list_response = app
        .clone()
        .oneshot(get(&format!(
            "/api/v1/communications/telegram/chats/{telegram_chat_id}/topics?limit=10"
        )))
        .await
    ...
    let create_response = app
        .clone()
        .oneshot(json_post(
            &format!("/api/v1/communications/telegram/chats/{telegram_chat_id}/topics"),
            json!({
                "account_id": "acct-1",
                "provider_chat_id": "provider-chat-1",
                "title": "New Topic",
    ...
    let close_response = app
        .clone()
        .oneshot(json_post(
            "/api/v1/communications/telegram/topics/topic-1/close",
            json!({
                "account_id": "acct-1",
                "provider_chat_id": "provider-chat-1",
                "is_closed": true,
