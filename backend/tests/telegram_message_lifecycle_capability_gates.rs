...
async fn removed_account_blocks_message_lifecycle_and_reaction_routes_before_side_effects() {
    ...
    let edit_response = app
        .clone()
        .oneshot(json_post_request_with_actor(
            &format!("/api/v1/telegram/messages/{message_id}/edit"),
            json!({
    ...
    let delete_response = app
        .clone()
        .oneshot(json_post_request_with_actor(
            &format!("/api/v1/telegram/messages/{message_id}/delete"),
            json!({
    ...
    let remove_reaction_response = app
        ...
        .oneshot(delete_request_with_token(
            &format!(
                "/api/v1/telegram/messages/{message_id}/reactions?account_id={account_id}&provider_chat_id={provider_chat_id}&provider_message_id={provider_message_id}&reaction_emoji=%F0%9F%91%8D&sender_id=owner&sender_display_name=Owner&command_id=unreact-{suffix}"
            ),
            LOCAL_API_TOKEN,
        ))
        .await
