...
async fn telegram_restore_and_reaction_actions_record_durable_command_rows() {
    ...
    let remove_reaction_response = app
        ...
        .oneshot(delete_request_with_token(
            &format!(
                "/api/v1/communications/telegram/messages/{message_id}/reactions?account_id={account_id}&provider_chat_id={chat_id}&provider_message_id={provider_message_id}&reaction_emoji=%F0%9F%91%8D&sender_id=owner&sender_display_name=Owner&command_id=unreact-{suffix}"
            ),
            LOCAL_API_TOKEN,
        ))
        .await
