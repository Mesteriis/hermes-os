...
test('authenticated browser device context reaches the owner-neutral module envelope', () => {
  const protocol = read(
  ...
  assert.match(
    validation,
    /logical_owner_id\.is_empty\(\) != request\.authenticated_device_id\.is_empty\(\)/,
  );
  assert.match(gateway, /let device_id = session\.device_id\(\)\.to_owned\(\)/);
  assert.match(
    gateway,
    /handler\(&route, &owner_id, &device_id, &body\)/,
  );
  assert.match(
    kernel,
