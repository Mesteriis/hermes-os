...
test('Mail operational read contract is typed, bounded and not falsely admitted', async () => {
  const [adr, inventorySource, proto, validator, build, api] = await Promise.all(
  ...
  assert.match(
    adr,
    /Mail persistence, sync materialization, runtime\s+route, managed conformance и frontend cutover ещё не реализованы/,
  );
