...
async fn task_candidates_reject_missing_local_api_token() {
    ...
    let response = app
        .oneshot(get_request("/api/v2/task-candidates"))
        .await
        .expect("response");
...
async fn task_candidates_returns_safe_candidate_payload() {
    ...
    let response = app
        .oneshot(get_request_with_token(
            &format!("/api/v2/task-candidates?limit={}&", 100),
            LOCAL_API_TOKEN,
        ))
        .await
        .expect("response");
...
async fn put_task_candidate_review_requires_actor_and_confirms_task() {
    ...
    let missing_actor = app
        ...
        .oneshot(json_put_request_with_token(
            &format!("/api/v2/task-candidates/{task_candidate_id}/review"),
            json!({
        ...
        .await
        .expect("response");
    // In this API layer, missing actor IDs are surfaced as invalid_actor_id.
...
async fn put_task_candidate_review_rejects_missing_candidate() {
    ...
    let response = app
        .oneshot(json_put_request_with_actor(
            "/api/v2/task-candidates/task_candidate:v1:missing%3Amissing%3Acandidate/review",
            json!({
