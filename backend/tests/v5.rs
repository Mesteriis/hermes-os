...
#[test]
fn v5_provider_and_secret_kinds_are_account_scoped() {
    assert_eq!(
...
#[tokio::test]
async fn v5_whatsapp_web_session_metadata_is_account_scoped_against_postgres() {
    let Some(database_url) = env::var("HERMES_TEST_DATABASE_URL").ok() else {
        eprintln!("skipping live V5 WhatsApp Web smoke test: HERMES_TEST_DATABASE_URL is not set");
        return;
    };
...
#[tokio::test]
async fn v5_api_exercises_whatsapp_web_fixture_foundation() {
    let Some(database_url) = env::var("HERMES_TEST_DATABASE_URL").ok() else {
        eprintln!("skipping live V5 API smoke test: HERMES_TEST_DATABASE_URL is not set");
        return;
    };
    let database = Database::connect(Some(&database_url))
        .await
    ...
    let message_response = app
        ...
        .oneshot(json_post_request_with_actor(
            ...
            json!({
                ...
                "text": "Please carry V5 WhatsApp Web context into graph-backed recall.",
    ...
    let sessions_response = app
        .clone()
        .oneshot(get_request_with_token(
            &format!("/api/v5/whatsapp/sessions?account_id={account_id}"),
            LOCAL_API_TOKEN,
        ))
        .await
    ...
    let messages_response = app
        ...
        .oneshot(get_request_with_token(
            &format!(
                "/api/v5/whatsapp/messages?account_id={account_id}&provider_chat_id={chat_id}"
            ),
            LOCAL_API_TOKEN,
        ))
        .await
