...
use crate::errors::ObservationValidationError;
...
impl ObservationOriginKind {
    ...
    pub fn parse(value: impl AsRef<str>) -> Result<Self, ObservationValidationError> {
        match value.as_ref() {
            ...
            unknown => Err(ObservationValidationError::UnknownOriginKind(
                unknown.to_owned(),
            )),
        }
    }
...
impl ObservationIngestionRunStatus {
    ...
    pub fn parse(value: impl AsRef<str>) -> Result<Self, ObservationValidationError> {
        match value.as_ref() {
            ...
            unknown => Err(ObservationValidationError::UnknownIngestionRunStatus(
                unknown.to_owned(),
            )),
        }
    }
...
pub fn validate_non_empty(
    ...
    if value.trim().is_empty() {
        return Err(ObservationValidationError::EmptyField(field_name));
    }
...
pub fn validate_json_object(
    ...
    if !value.is_object() {
        return Err(ObservationValidationError::InvalidJsonObject(field_name));
    }
...
pub fn validate_score(
    ...
    if !(0.0..=1.0).contains(&value) {
        return Err(ObservationValidationError::InvalidScore(field_name, value));
    }
