...
const MAIL_SYNC_HEALTH_QUERY_CAPABILITY = 'mail.sync.health.query.v1'
const MAX_CONNECTION_ID_BYTES = 512
const textEncoder = new TextEncoder()

export type MailSyncHealthConnection = {
	connectionId: string
	registrationId: string
}

export function mailSyncHealthConnections(
	modules: readonly ClientModuleBootstrapV1[],
): readonly MailSyncHealthConnection[] {
	const connections = new Map<string, MailSyncHealthConnection>()
	for (const module of modules) {
		if (
			module.moduleId !== MAIL_MODULE_ID
			|| !module.sectionsEnabled
			|| !module.capabilityIds.includes(MAIL_SYNC_HEALTH_QUERY_CAPABILITY)
		) continue
...
export function mailSyncHealthConnectionFingerprint(
	modules: readonly ClientModuleBootstrapV1[],
): string {
	return mailSyncHealthConnections(modules)
