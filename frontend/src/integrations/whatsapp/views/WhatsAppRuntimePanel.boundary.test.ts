...
describe('WhatsAppRuntimePanel boundary', () => {
  it('preserves projected sync snapshot orchestration after removing the runtime panel Vue layer', () => {
    ...
    expect(surfaceSource).toContain('useWhatsappSyncPresenceQuery(selectedAccountId, selectedSyncChatIdResolved, 50)')
    expect(surfaceSource).toContain('useWhatsappSyncCallsQuery(selectedAccountId, selectedSyncChatIdResolved, 50)')
    expect(surfaceSource).toContain('useWhatsappSyncMediaQuery(selectedAccountId, selectedSyncChatIdResolved, 50)')
