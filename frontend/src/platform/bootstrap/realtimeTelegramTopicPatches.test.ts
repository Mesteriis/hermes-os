...
describe('telegram realtime topic cache patch handling', () => {
  it('patches cached telegram topic lists for topic update events', () => {
    const topicsKey = ['telegram', 'topics', 'telegram-chat-1', 100]
    const topicSearchKey = ['telegram', 'topic-search', 'telegram-chat-1', 'release', 50]
    const existingTopic = {
    ...
    const queryClient = {
      ...
      getQueriesData: vi.fn().mockImplementation(({ queryKey }) => {
        if (JSON.stringify(queryKey) === JSON.stringify(['telegram'])) {
    ...
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({ queryKey: ['telegram', 'topics'] })
    expect(queryClient.invalidateQueries).toHaveBeenCalledWith({ queryKey: ['telegram', 'topic-search'] })
  })
})
