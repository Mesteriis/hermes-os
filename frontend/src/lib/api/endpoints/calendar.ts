...
export async function fetchEventChecklist(eventId: string): Promise<EventChecklist | null> {
	return ApiClient.instance.get<EventChecklist | null>(
		`/api/v1/calendar/events/${encodeURIComponent(eventId)}/checklist`,
...
export async function fetchFocusBlocks(): Promise<FocusBlocksResponse> {
	return ApiClient.instance.get<FocusBlocksResponse>('/api/v1/calendar/focus-blocks', 'Focus blocks request failed');
}

export async function fetchCalendarWatchtower(): Promise<Record<string, unknown>> {
	return ApiClient.instance.get<Record<string, unknown>>('/api/v1/calendar/watchtower', 'Watchtower request failed');
}

export async function fetchWeeklyBrief(): Promise<Record<string, unknown>> {
	return ApiClient.instance.get<Record<string, unknown>>('/api/v1/calendar/weekly-brief', 'Weekly brief request failed');
}

export async function fetchCalendarRules(): Promise<CalendarRulesResponse> {
	return ApiClient.instance.get<CalendarRulesResponse>('/api/v1/calendar/rules', 'Rules request failed');
}

export async function postCalendarBrain(question: string): Promise<Record<string, unknown>> {
	return ApiClient.instance.post<Record<string, unknown>>(
		'/api/v1/calendar/brain',
		{ q: question },
		'Brain query failed'
	);
}
