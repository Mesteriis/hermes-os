<script lang="ts">
	...
	import type { AiRun, AiStatus, AiCitation, OwnerPersona } from '$lib/api';
...
<aside class="stacked-rail">
	<div class="widget-frame" class:editing={isLayoutEditing} data-widget-id="ai-runtime-status" data-widget-hidden={!isWidgetVisible('ai-runtime-status')}>
		...
		<section class="panel info-card"><h2>Runtime</h2><div class="health-row"><span>Status</span><strong>{aiRuntimeSummary()}</strong></div><div class="health-row"><span>Owner Persona</span><strong>{ownerPersona?.display_name ?? 'not set'}</strong></div><div class="health-row"><span>Chat</span><strong>{aiStatus?.chat_model ?? 'unknown'}</strong></div><div class="health-row"><span>Embedding</span><strong>{aiStatus?.embedding_model ?? 'unknown'}</strong></div></section>
	</div>
