		...
		{#if selectedWhatsappSession}
			...
			<header>
				...
				<div class="chat-actions">
					<button type="button" disabled title={_('Live WhatsApp Web runtime is blocked in WhatsApp foundation')}><Icon icon="tabler:world" width="17" height="17" /></button>
					<button type="button" disabled title={_('Outbound WhatsApp sends require a future policy and runtime contract')}><Icon icon="tabler:send-off" width="17" height="17" /></button>
			...
			<div class="chat-body">
				...
				{#if selectedWhatsappMessages.length === 0}
					<div class="empty-panel fill">{_('No WhatsApp Web messages for this session.')}</div>
				{:else}
			...
			<form class="telegram-inline-form" onsubmit={(event) => { event.preventDefault(); void ingestWhatsappWebMessageFixture(); }}>
				<input bind:value={whatsappMessageForm.provider_message_id} placeholder={_('Provider message ID')} autocomplete="off" />
				<input bind:value={whatsappMessageForm.sender_display_name} placeholder={_('Sender')} autocomplete="off" />
				<input bind:value={whatsappMessageForm.text} placeholder={_('Fixture message text')} autocomplete="off" />
				<button type="submit" disabled={isWhatsappActionSubmitting || !whatsappMessageForm.text.trim()}><Icon icon="tabler:send" width="17" height="17" />{_('Ingest')}</button>
			</form>
		{:else}
			<div class="empty-panel fill">{_('Create a WhatsApp Web fixture account before ingesting messages.')}</div>
		{/if}
	</section>
</div>
