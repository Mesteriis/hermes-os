...
<aside class="context-rail">
	<div class="widget-frame" class:editing={isLayoutEditing} data-widget-id="communications-sender-profile" data-widget-hidden={!isWidgetVisible('communications-sender-profile')}>
		...
		<section class="panel profile-panel">
			<div class="profile-head"><img src="/assets/hermes-reference-avatar.png" alt="" /><div><h2>{selectedCommunication ? senderLabel((selectedCommunication as Record<string, unknown>).sender as string) : _('No sender selected')}</h2><p>{selectedCommunication ? communicationChannelLabel((selectedCommunication as Record<string, unknown>).channel_kind as string) : _('No channel')}</p><small>{selectedCommunication ? senderEmail((selectedCommunication as Record<string, unknown>).sender as string) : _('No local message selected')}</small></div></div>
	...
	<div class="widget-frame" class:editing={isLayoutEditing} data-widget-id="communications-summary" data-widget-hidden={!isWidgetVisible('communications-summary')}>
		...
		<section class="panel info-card"><h2>{_('Summary')}</h2><p>{selectedCommunication ? `Stored from ${(selectedCommunication as Record<string, unknown>).account_id as string}. Channel ${communicationChannelLabel((selectedCommunication as Record<string, unknown>).channel_kind as string)}. Provider record ${(selectedCommunication as Record<string, unknown>).provider_record_id as string}.` : _('Local communication metadata will appear after messages are imported.')}</p><button type="button" class="link-row" disabled>{_('View full profile')} <Icon icon="tabler:arrow-right" width="15" height="15" /></button></section>
	</div>
	<div class="widget-frame" class:editing={isLayoutEditing} data-widget-id="communications-message-metadata" data-widget-hidden={!isWidgetVisible('communications-message-metadata')}>
		...
		<section class="panel info-card"><h2>{_('Message Metadata')}</h2>{#if selectedCommunication}<ul class="detail-list"><li><Icon icon="tabler:users" width="17" height="17" /> {((selectedCommunication as Record<string, unknown>).recipients as unknown[]).length} {_('recipients')}</li><li><Icon icon="tabler:paperclip" width="17" height="17" /> {(selectedCommunication as Record<string, unknown>).attachment_count as number} {_('attachments')}</li><li><Icon icon="tabler:clock" width="17" height="17" /> {messageTime(selectedCommunication)}</li></ul>{:else}<p>{_('No message selected.')}</p>{/if}</section>
	</div>
