	...
	<section class="panel org-detail-panel">
		{#if selectedOrganization}
			<header>
				...
				<div><h2>{selectedOrganization.display_name as string}</h2><em>{selectedOrganization.industry as string || 'Unknown industry'}{#if selectedOrganization.country} · {selectedOrganization.country as string}{/if}</em></div>
			</header>
			<div class="org-detail-grid">
				<div class="info-card"><h3>Status</h3><span class="status-chip {selectedOrganization.status as string}">{selectedOrganization.status as string}</span>{#if selectedOrganization.health_status}<span class="health-chip">{selectedOrganization.health_status as string}</span>{/if}{#if selectedOrganization.watchlist}<span class="health-chip important">Watchlist</span>{/if}</div>
				{#if selectedOrganization.description}
					<div class="info-card"><h3>About</h3><p>{selectedOrganization.description as string}</p></div>
				{/if}
				<div class="info-card"><h3>Details</h3>
					{#if selectedOrganization.website}<div class="detail-row"><span>Website</span><strong>{selectedOrganization.website as string}</strong></div>{/if}
					{#if selectedOrganization.legal_name}<div class="detail-row"><span>Legal name</span><strong>{selectedOrganization.legal_name as string}</strong></div>{/if}
					{#if selectedOrganization.registration_number}<div class="detail-row"><span>Registration</span><strong>{selectedOrganization.registration_number as string}</strong></div>{/if}
					{#if selectedOrganization.vat}<div class="detail-row"><span>VAT</span><strong>{selectedOrganization.vat as string}</strong></div>{/if}
					<div class="detail-row"><span>Interactions</span><strong>{selectedOrganization.interaction_count as string}</strong></div>
					<div class="detail-row"><span>Priority</span><strong>{selectedOrganization.priority as string || 'normal'}</strong></div>
				</div>
