...
describe('AppNavbarRouteBreadcrumbs boundary', () => {
	it('keeps route breadcrumb dropdowns as a prop-driven layout component', () => {
		...
		expect(source).toContain('v-else-if="isLastLevel(index)"')
		expect(source).toContain('class="app-navbar-route-breadcrumbs__label"')
		expect(source).toContain('level.currentItem.loading')
