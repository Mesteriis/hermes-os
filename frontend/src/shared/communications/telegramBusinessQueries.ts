...
} from '../../integrations/telegram/types/telegram'
import type { TelegramRawMessageResponse } from '../../integrations/telegram/types/telegramRawEvidence'
...
export function useReplyTelegramMessageMutation() {
  ...
  return useMutation({
    mutationFn: (request: {
      ...
      text: string
    }) => replyToTelegramBusinessMessage({ message_id: request.message_id, text: request.text }),
    onSuccess: invalidate,
  })
}
