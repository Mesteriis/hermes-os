...
describe('MaintenanceSettings boundary', () => {
  it('keeps Maintenance as a Settings-owned backend surface with guarded actions', () => {
    ...
    const actions = readFileSync(new URL('../queries/maintenanceSettingsActions.ts', import.meta.url), 'utf8')
    ...
    expect(pagePresentation).toContain("id: 'maintenance'")
    expect(pagePresentation).toContain("label: 'Maintenance'")
    ...
    expect(actions).toContain('canRunMaintenanceAction')
    expect(actions).toContain('runSelectedMaintenanceAction')
    expect(panel).toContain('settings-maintenance-confirm')
    expect(panel).toContain('surface.canRunSelectedAction')
    expect(panel).toContain('useMaintenanceSettingsPanelController')
    expect(panel).toContain('handleRefresh')
    expect(panel).toContain('handleSelectAction')
    expect(panel).toContain('handleConfirmationInput')
    expect(panel).toContain('handleRunSelectedAction')
    expect(panel).not.toContain('eventValue(')
    expect(panel).not.toContain('surface.handleRefresh()')
