...
export async function fetchMessageSignatureConnect(
  ...
  const response = await getCommunicationsConnectClient().getMessageSignature({ messageId })
  return {
...
export async function extractMessageTasksConnect(messageId: string): Promise<ExtractTasksResponse> {
  const response = await getCommunicationsConnectClient().extractMessageTasks({ messageId })
  return {
...
export async function extractMessageNotesConnect(messageId: string): Promise<ExtractNotesResponse> {
  const response = await getCommunicationsConnectClient().extractMessageNotes({ messageId })
  return {
