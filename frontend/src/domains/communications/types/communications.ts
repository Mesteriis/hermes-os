...
export type CommunicationMessageSummary = {
  ...
  is_read?: boolean;
  read_changed_at?: string | null;
  read_origin?: string;
  read_sync_status?: MailReadSyncStatus;
};
...
export type ComposeAttachmentModel = {
  attachmentId: string;
  filename: string;
  contentType: string;
  sizeBytes: number;
  scanStatus: string;
  uploadStatus: "uploading" | "ready" | "blocked" | "failed";
  error: string;
};
...
export type ComposeFormModel = {
  ...
  attachments: ComposeAttachmentModel[];
};
