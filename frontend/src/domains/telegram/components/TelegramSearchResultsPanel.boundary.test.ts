...
describe('TelegramSearchResultsPanel media boundary', () => {
  it('allows projected media search hits to reopen owning message context without inline fetch', () => {
    ...
    expect(source).toContain('function mediaReadiness(item: TelegramMediaItem)')
    expect(source).toContain("return telegramMediaReadiness(item)")
    expect(source).toContain(':title="mediaReadiness(item).detail"')
    expect(source).toContain('{{ mediaReadiness(item).label }} · {{ mediaReadiness(item).detail }}')
