...
describe('TelegramMessageReferencePanel boundary', () => {
  it('renders projected lifecycle and reference evidence instead of raw provider ids only', () => {
    ...
    expect(source).toContain('providerChatId: computed(() => props.currentMessage.provider_chat_id)')
    expect(source).toContain('providerMessageId: computed(() => props.currentMessage.provider_message_id)')
    ...
    expect(source).toContain('const relatedCommands = computed(() => (commandsQuery.data.value ?? []).slice(0, 5))')
