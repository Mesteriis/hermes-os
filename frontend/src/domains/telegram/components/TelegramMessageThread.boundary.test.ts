...
describe('TelegramMessageThread thread surfaces boundary', () => {
  it('uses query-backed pinned messages, exposes a dedicated voice tab, and forwards media search openings', () => {
    ...
    expect(source).toContain('telegramWorkspaceSearchSourceLabel')
    expect(source).toContain(':sourceLabel="telegramWorkspaceSearchSourceLabel(selectedTelegramRuntimeStatus)"')
    expect(source).toContain(':mediaSourceLabel="mediaSearchSourceLabel"')
    expect(source).toContain(':accountId="selectedTelegramChat.account_id"')
    expect(source).toContain('TelegramSavedSearchStrip')
    expect(source).toContain(':currentQuery="workspaceSearchQuery"')
    ...
    expect(source).toContain('@uploadMedia="(file) => emit(\'uploadMedia\', file)"')
  })
})
