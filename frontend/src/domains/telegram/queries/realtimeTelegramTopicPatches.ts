...
function isTopicListQueryKey(queryKey: readonly unknown[]): boolean {
  if (queryKey[0] !== 'telegram') return false
  return queryKey[1] === 'topics' || queryKey[1] === 'topic-search'
}

function topicSearchQuery(queryKey: readonly unknown[]): string {
  if (queryKey[1] !== 'topic-search') return ''
  return typeof queryKey[3] === 'string' ? queryKey[3].trim().toLowerCase() : ''
}
