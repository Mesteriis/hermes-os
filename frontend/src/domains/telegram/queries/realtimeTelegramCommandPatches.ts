import { isRecord, numberValue, stringValue } from '../../communications/queries/realtimePatchShared'
...
export function patchTelegramCommandList(
  ...
  const queryProviderChatId =
    typeof queryKey[4] === 'string' && queryKey[4] !== 'all' ? queryKey[4] : null
  const queryProviderMessageId =
    typeof queryKey[5] === 'string' && queryKey[5] !== 'all' ? queryKey[5] : null
  const queryCommandKinds =
    typeof queryKey[6] === 'string' && queryKey[6] !== 'all'
      ? new Set(queryKey[6].split('|').filter((value) => value.length > 0))
      : null
