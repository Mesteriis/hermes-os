...
describe('OrganizationsPage boundary', () => {
	it('preserves organizations selection orchestration after removing the legacy OrganizationsPage Vue layer', () => {
		const appSurfaceSource = readFileSync(new URL('../../../app/queries/useOrganizationsViewSurface.ts', import.meta.url), 'utf8')
    ...
    expect(existsSync(new URL('../components/OrganizationsDetail.vue', import.meta.url))).toBe(false)
		...
		expect(appSurfaceSource).toContain('Organizations UI removed after logic extraction. Rebuild pending new design language.')
		expect(appSurfaceSource).toContain('Organizations logic is preserved')
