...
export const MailList: Story = {
	...
	play: async ({ canvasElement, globals }) => {
		...
		await expect(canvas.getAllByRole('button', { name: text.settings }).at(-1)).toBeVisible()
		await expect(canvas.getByRole('combobox', { name: text.mailView })).toBeVisible()
...
export const MailInspector: Story = {
	...
	play: async ({ canvasElement, globals }) => {
		...
		await expect(canvas.getAllByText('Retention clause approval')[0]).toBeVisible()
...
export const MailWorkspace: Story = {
	...
	play: async ({ canvasElement, globals }) => {
		...
		await expect(canvas.getAllByRole('region', { name: text.openMessage })[0]).toBeVisible()
		await expect(canvas.getByRole('complementary', { name: text.mailInspector })).toBeVisible()
		await userEvent.click(canvas.getByRole('button', { name: text.hideHermesInspector }))
